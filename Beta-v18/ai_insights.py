...
class AIInsights:
    ...
    def ask_wazuh_question(self, question, context_data=None, language=None):
        ...
        """
        system_prompt = """You are an expert cybersecurity analyst specializing in Wazuh SIEM.

CRITICAL INSTRUCTIONS:
1. ALWAYS respond in English. Regardless of the user's input language, the response must be in English.
2. NARRATIVE STYLE ONLY: You MUST provide all analysis in a highly readable, structured narrative format. Do NOT use tables under any circumstances, even for FIM (File Integrity Monitoring) or timeline data. Use bold headers, bullet points, and paragraphs to organize information.
3. INTELLIGENT DELETION TRACKING: When a user asks "who deleted [filename]", specifically look for:
   - Wazuh rule IDs 550, 553, 554 (syscheck events)
   - Actions like "deleted", "removed", or "File deleted" in the event description or full_log.
5. ADDITIONAL ANALYSIS DOMAINS:
   - Authentication & Authorization: Analyze login failures, brute force, and unauthorized access attempts.
   - Malware & Persistence: Identify suspicious processes, service creations, and startup modifications.
   - Privilege Escalation: Look for sudo usage, elevation of privileges, and account modifications.
   - Network Threats: Detect port scanning, unusual traffic patterns, and connection attempts to malicious IPs.
   - Log Integrity: Identify gaps in logs or attempts to clear event logs.
   - Misconfiguration: Spot weak passwords, unhardened systems, and policy violations.
   - RDP Security: Focus on remote desktop connection attempts and potential hijacking.
   - Vulnerability & Compliance (SOC2/ISO): Evaluate system readiness for compliance audits. Analyze patch levels, configuration gaps, access controls, and encryption status. Provide guidance on SOC2 Trust Services Criteria (Security, Availability, Processing Integrity, Confidentiality, Privacy) based on the observed data.
6. Minimize descriptions. Be concise and focus on the data.
7. Base your answer DIRECTLY on the alert data provided, ensuring you scan ALL fields including "agent.name", "agent.ip", "full_log", "syscheck.path", and "data.win.eventdata".
8. For compliance-related questions (e.g., "Are we SOC2 compliant?"), analyze the available logs for evidence of required controls (access monitoring, change management, encryption) and identify specific gaps or risks that would affect eligibility.
9. Provide CONCRETE FINDINGS from the data: usernames, IP addresses, timestamps, agents, and specific file paths.
10. Format the response using Markdown with bold headers and narrative lists. Do NOT use tables. """
        ...
        if context_data:
            ...
            user_content = f"SECURITY ALERT DATA FROM SYSTEM (LAST 90 DAYS):\n{context_str}\n\n---\n\nANALYZE THE ABOVE DATA AND ANSWER THIS QUESTION IN ENGLISH:\n{question}"
        else:
            user_content = f"Question: {question}"
        
        # Build the ordered list of providers to try: primary first, then all others as fallback
        all_providers = ["openai", "gemini", "deepseek", "ollama"]
        provider_order = [self.model_type] + [p for p in all_providers if p != self.model_type]

        result = {"error": f"Primary provider '{self.model_type}' not attempted"}
        for provider in provider_order:
            try:
                ...
                logger.error(f"Provider '{provider}' raised exception: {e}")
                result = {"error": str(e)}

            if "error" not in result:
                if provider != self.model_type:
                    result["provider_used"] = f"{provider} (fallback)"
                    logger.info(f"Q&A succeeded with fallback provider: {provider}")
                return result
            else:
                logger.warning(f"Provider '{provider}' failed: {result.get('error', 'unknown error')}")

        # All providers failed
        return {
            "error": (
                "No AI provider is currently available. Please configure an API key for OpenAI, "
                "Gemini, or DeepSeek in the Admin → Configuration section, or ensure Ollama is running."
            )
        }
    ...
    def analyze_alerts(self, alerts_data, analysis_prompt=None, fields=None):
        ...
        for provider in provider_order:
            ...
            if "error" not in result:
                if provider != self.model_type:
                    result["provider_used"] = f"{provider} (fallback)"
                    logger.info(f"Analysis succeeded with fallback provider: {provider}")
                return result
            else:
                logger.warning(f"Provider '{provider}' failed: {result.get('error', 'unknown error')}")

        # All providers failed — return a user-friendly error
        result = {
            "error": (
                "No AI provider is currently available. Please configure an API key for OpenAI, "
                "Gemini, or DeepSeek in the Admin → Configuration section, or ensure Ollama is running."
