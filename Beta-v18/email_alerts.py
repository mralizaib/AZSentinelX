...
class EmailAlerts:
    ...
    def send_severity_alert(self, alert_config, alerts_data=None):
        ...
        try:
            ...
            # include_fields is already defined above

            # --- Correlated threat intel items with patches available ---
            ai_analysis_html = ''
            try:
                ...
                for ti in patch_items:
                    ...
                    rows.append(f"""
                      <tr>
                        <td style="padding:8px;border-bottom:1px solid #dee2e6;font-weight:bold;color:{sev_color}">{ti.severity.upper()}</td>
                        <td style="padding:8px;border-bottom:1px solid #dee2e6">{ti.title[:100]}</td>
                        <td style="padding:8px;border-bottom:1px solid #dee2e6;color:#555">{affected_note}</td>
                        <td style="padding:8px;border-bottom:1px solid #dee2e6">{env_action}</td>
                      </tr>""")

                if rows:
                    table_rows = "\n".join(rows)
                    ai_analysis_html = f"""
                <div style="margin:20px 0;padding:16px;background:#fff3cd;border-left:5px solid #fd7e14;border-radius:4px">
                  <h2 style="color:#856404;margin-top:0">&#128994; Patch-Available Threat Intelligence (Last 7 Days)</h2>
                  <p style="color:#555;margin-bottom:12px">The following correlated threats have confirmed patches available and require action:</p>
                  <table style="width:100%;border-collapse:collapse;font-size:13px">
                    <thead>
                      <tr style="background:#f8f9fa">
                        <th style="padding:8px;text-align:left;border-bottom:2px solid #dee2e6">Severity</th>
                        <th style="padding:8px;text-align:left;border-bottom:2px solid #dee2e6">Threat</th>
                        <th style="padding:8px;text-align:left;border-bottom:2px solid #dee2e6">Exposure</th>
                        <th style="padding:8px;text-align:left;border-bottom:2px solid #dee2e6">Recommended Action</th>
                      </tr>
                    </thead>
            ...
            # Inject AI analysis block (if available)
            body += ai_analysis_html
