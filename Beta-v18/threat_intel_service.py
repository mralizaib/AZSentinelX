...
def fetch_wazuh_cti(limit=50) -> list:
    """
    Harvest CVE vulnerability detections from Wazuh/OpenSearch that were
    originally sourced from cti.wazuh.com. This gives us the real-time
    vulnerability picture from your own environment.
    """
    items = []
    try:
        ...
        query = {
            ...
            "query": {
                "bool": {
                    "must": [
                        {"terms": {"rule.groups": ["vulnerability-detector", "vulnerability"]}},
        ...
        for hit in hits:
            ...
            cvss3 = vuln.get('cvss', {}).get('cvss3', {}).get('base_score', 0) or 0
            cvss2 = vuln.get('cvss', {}).get('cvss2', {}).get('base_score', 0) or 0
            cvss_score = cvss3 or cvss2

            pkg_name = vuln.get('package', {}).get('name', '')
            pkg_ver = vuln.get('package', {}).get('version', '')
            agent_name = src.get('agent', {}).get('name', '')
            description = vuln.get('description', '') or title_raw
            ref_url = vuln.get('reference', '') or f"https://cti.wazuh.com/vulnerabilities/cves/{cve_id}"
            published = _parse_date(vuln.get('published') or src.get('@timestamp', ''))

            title = f"{cve_id}"
            if title_raw and title_raw != cve_id:
                title += f" – {title_raw[:120]}"

            desc_parts = [description[:800]]
            if pkg_name:
                desc_parts.append(f"Affected package: {pkg_name} {pkg_ver}".strip())
            if agent_name:
                desc_parts.append(f"Detected on agent: {agent_name}")
            if cvss_score:
            ...
            items.append({
                ...
                'published_at': published,
                'severity': severity,
                'has_patch': _detect_patch(description),
                'has_mitigation': _detect_mitigation(description),
                'cve_ids': [cve_id],
            })

    except Exception as e:
        logger.error(f"Error fetching Wazuh CTI vulnerabilities from OpenSearch: {e}")
    return items
...
def send_threat_email(item, analysis: dict, recipient: str) -> bool:
    ...
    try:
        ...
        logger.info(f"Threat intel email sent to {recipient} for: {item.title[:60]}")
        return True
    except Exception as e:
...
"""
    ...
    try:
        ...
        with smtplib.SMTP(Config.SMTP_SERVER, Config.SMTP_PORT) as server:
            ...
            server.login(Config.SMTP_USERNAME, Config.SMTP_PASSWORD)
            server.sendmail(Config.SMTP_USERNAME, recipient, msg.as_string())
        logger.info(f"Threat intel email sent to {recipient}: {item.title[:60]}")
        return True
    except Exception as e:
        logger.error(f"Failed to send threat intel email: {e}")
        return False
