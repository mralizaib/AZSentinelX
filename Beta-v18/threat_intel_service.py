...
def fetch_wazuh_cti(limit=500) -> list:
    ...
    try:
        ...
        _agent_sub_agg = {"terms": {"field": "agent.name", "size": 50}}
            ...
            "aggs": {
                "unique_cves": {
                    ...
                    "aggs": {
                        ...
                        # Active: still unpatched on these agents
                        "active_agents": {
                            "filter": {"terms": {"data.vulnerability.status": ["Active", "active"]}},
                            "aggs": {"by_agent": _agent_sub_agg},
                        },
                        # Solved: patch already applied on these agents
                        "solved_agents": {
