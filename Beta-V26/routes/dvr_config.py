...
def configure_users():
    ...
    # Manager account flag: explicit boolean or presence of 'manager' in selected_users
    create_manager = bool(data.get('create_manager', False)) or ('manager' in selected_users)
    ...
    manager_password = f"{initials}_man12"
    ...
    try:
        ...
        if 'manager' in selected_users:
            try:
                mgr_id = client.create_user("manager", manager_password, role="viewer")
                perm_ok = client.set_user_permissions(mgr_id, "manager", "manager")
                results.append({
                    'username': 'manager',
