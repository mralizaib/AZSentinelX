...
def _patch_xml_field(xml_str, tag, new_value):
    ...
    This approach is safer than ElementTree round-tripping for OEM firmware
    because it leaves the original document structure, namespace declarations,
    and element ordering 100% intact — only the specific field value changes.
    """
    import re as _re
    pattern = rf"(<{_re.escape(tag)}(?:\s[^>]*)?>).*?(</{_re.escape(tag)}>)"
    replacement = rf"\g<1>{new_value}\g<2>"
    return _re.sub(pattern, replacement, xml_str, flags=_re.DOTALL)
...
<storage>
  <hddList>
    ...
    def configure_main_stream(self, channel_id):
        ...
        1920x1080, H.264, VBR, 320kbps, 8fps (maxFrameRate=800).
        Regex patching leaves the original XML document byte-for-byte intact
        except for the specific field values changed, so OEM/std-cgi firmware
        namespace validation always passes.
        """
        try:
            r = self._get(f"/Streaming/channels/{channel_id}")
    ...
    def configure_sub_stream(self, channel_id):
        ...
        960x576, H.264, VBR, 320kbps, 8fps.
