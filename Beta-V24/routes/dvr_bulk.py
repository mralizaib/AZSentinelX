...
def _configure_one_device(app, job_id, device_idx, device, global_city, global_state):
    ...
    try:
        ...
        channel_ids = info.get('channel_ids') or []
        if not channel_ids:
            total_ch = int(info.get('total_channels', 0)) or 4
            channel_ids = [f"{ch}01" for ch in range(1, total_ch + 1)]
        stream_results = []
        for main_id in channel_ids:
            ...
            stream_results.append({'channel': ch_label,
                                   'main': 'ok' if main_ok else 'error',
                                   'sub':  'ok' if sub_ok  else 'error'})
        client.disable_channel_zero()
        result['steps']['streams'] = {'status': 'done', 'channels': stream_results}
        ...
        for main_id in channel_ids:
            ...
            rec_results.append({'channel': ch_label, 'status': 'ok' if ok else 'error'})
        result['steps']['recording'] = {'status': 'done', 'channels': rec_results}
        ...
        result['steps']['users'] = {'status': 'done', 'accounts': user_results,
        ...
        with app.app_context():
            ...
            sess_data = {
                ...
                'steps': {
                    ...
                    'streams': {'channels': stream_results, 'channel_zero_disabled': True},
                    'recording': {'channels': rec_results},
...
def _build_bulk_excel(job_data, job):
    ...
    for d in devices:
        ...
        if checks:
            ws2.append(['Validation Check', 'Status', 'Detail'])
            for cell in ws2[ws2.max_row]:
                cell.font = hdr_font
                cell.fill = hdr_fill
            for c in checks:
                icon = {'pass': '✅', 'warn': '⚠', 'fail': '❌'}.get(c['status'], '?')
                ws2.append([c['check'], f"{icon} {c['status'].upper()}", c['detail']])
...
def _build_bulk_html_summary(job_data, job):
    """Build a standalone HTML summary page for the entire bulk job."""
    devices = job_data.get('devices', [])
    total = len(devices)
...
<h1>🎥 ByteIT SentinalX — Bulk DVR/NVR Configuration Report</h1>
...
<p style="color:#999;font-size:0.85em;text-align:center;">
  ByteIT SentinalX — AI DVR/NVR Bulk Auto-Configuration &nbsp;|&nbsp; {ts} UTC
</p>
</body></html>"""
