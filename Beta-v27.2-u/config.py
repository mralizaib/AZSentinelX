...
class Config:
    ...
    OPENSEARCH_INDEX_PATTERN = os.environ.get('OPENSEARCH_INDEX_PATTERN',
                                              'wazuh-alerts-*')
    OPENSEARCH_ARCHIVES_INDEX_PATTERN = os.environ.get('OPENSEARCH_ARCHIVES_INDEX_PATTERN',
                                              'wazuh-archives-*')
    ...
    SMTP_USERNAME = os.environ.get('SMTP_USERNAME', 'italerts.hyd.pak@oxiliry.com')
    SMTP_PASSWORD = os.environ.get('SMTP_PASSWORD', 'omdtmsquvabmqpxq')
    ...
    def get_wazuh_servers(cls):
        ...
        return {
            'primary': {
                'key': 'primary',
                'name': os.environ.get('WAZUH_SERVER_1_NAME', 'Pakistan Site'),
                'timezone': os.environ.get('WAZUH_SERVER_1_TIMEZONE', 'Asia/Karachi'),
                ...
                'wazuh_password': os.environ.get('WAZUH_API_PASSWORD', 'Jbbp1P*9ydI*EP7.Wa2MCLsKM?lcz+iH'),
                'wazuh_verify_ssl': os.environ.get('WAZUH_VERIFY_SSL', 'False') == 'True',
            ...
            'secondary': {
                'key': 'secondary',
                'name': os.environ.get('WAZUH_SERVER_2_NAME', 'US Site'),
                'timezone': os.environ.get('WAZUH_SERVER_2_TIMEZONE', 'America/New_York'),
                'wazuh_url': os.environ.get('WAZUH_API_URL_2', 'https://10.1.1.69:55000').rstrip('/'),
                'wazuh_user': os.environ.get('WAZUH_API_USER_2', 'wazuh-wui'),
                'wazuh_password': os.environ.get('WAZUH_API_PASSWORD_2', 'XpN7ZIgNFXFz2h8XM1ojq83YqqB+uaZr'),
                'wazuh_verify_ssl': os.environ.get('WAZUH_VERIFY_SSL_2', 'False') == 'True',
                'opensearch_url': os.environ.get('OPENSEARCH_URL_2', 'https://10.1.1.69:9200').rstrip('/'),
                'opensearch_user': os.environ.get('OPENSEARCH_USER_2', 'admin'),
                'opensearch_password': os.environ.get('OPENSEARCH_PASSWORD_2', 'ReBizus*1786'),
    ...
    @classmethod
    def get_configured_servers(cls):
        """Return only server profiles that have complete Wazuh/OpenSearch settings."""
        return [
            profile
            for profile in cls.get_wazuh_servers().values()
            if profile['wazuh_url'] and profile['wazuh_user'] and
            profile['wazuh_password'] and profile['opensearch_url'] and
            profile['opensearch_user'] and profile['opensearch_password']
        ]
