...
class EmailAlerts:
    def __init__(self, server_key='primary'):
        ...
        self.opensearch = OpenSearchAPI(server_key=self.server_key)
        self.server_name = self.opensearch.server_name
        # Email-facing site names are intentionally independent from the
        # infrastructure labels ("Pakistan Site"/"USA Site") used in server
        # configuration and UI routing.
        self.site_name = {
            'primary': 'Oxiliry',
            'secondary': 'ReBiz',
        }.get(self.server_key, self.server_name)
        self.report_generator = ReportGenerator(server_key=self.server_key)
    ...
    def _build_html_email_body(self, total_alerts, alert_counts, severity_levels,
        ...
        for idx, alert in enumerate(results):
            ...
            if is_vuln:
                ...
                defender_status = normalize_defender_event(
                    (data_field.get('win', {}) or {}).get('eventdata', {}) or {},
                    rule_desc,
                )
                is_resolved = defender_status['action_status'] == 'COMPLETED'
                raw_status = defender_status['action_status_label']
            else:
                is_resolved = False
                raw_status  = ''
            ...
            if is_vuln:
                ...
                defender_status = normalize_defender_event(win_ed, rule_desc)
                threat   = defender_status['threat_name']
                action   = defender_status['action_type']
                fp       = pick_defender_field(
                    win_ed, 'path', 'Path', 'process', 'Process',
                    'filePath', 'file',
                )
                parts = []
                if threat:
                ...
                if action:
                    parts.append(f'<strong>Action:</strong> {action}')
                parts.append(
                    f"<strong>Status:</strong> {defender_status['action_status_label']}"
                )
                parts.append(
                    f"<strong>IT Action Required:</strong> "
                    f"{'Yes' if defender_status['additional_action_required'] else 'No'}"
                )
                if fp:
                    parts.append(f'<strong>Path:</strong> {fp}')
                if defender_status['execution_result']:
                    parts.append(
                        f"<strong>Result:</strong> {defender_status['execution_result']}"
                    )
                if parts:
                    detail_html = (
...
<title>Security Alert Notification &mdash; REBIZ Sentinel X</title>
</head>
...
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#eef2f7;">
  <tr>
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#0d1b4b 0%,#1a3a8f 100%);padding:18px 32px;border-radius:0 0 10px 10px;">
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
              <tr>
                <td valign="middle">
                  <div style="color:#ffffff;font-weight:700;font-size:13px;letter-spacing:0.3px;">REBIZ Sentinel X</div>
    ...
    def _generate_alert_identifier(self, alert_data):
        ...
        if event_id:
            fields = {'server_key': self.server_key, 'event_id': event_id}
        else:
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#0d1b4b 0%,#1a3a8f 100%);padding:18px 32px;border-radius:0 0 10px 10px;">
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
              <tr>
                <td valign="middle">
                  <div style="color:#ffffff;font-weight:700;font-size:13px;">REBIZ Sentinel X</div>
                  <div style="color:rgba(255,255,255,0.6);font-size:10px;margin-top:4px;">Automated Security Alert &middot; Do not reply to this email</div>
                  <div style="color:rgba(255,255,255,0.6);font-size:10px;margin-top:2px;">IT Department &middot; itsupport@rebiz.com &middot; Created by Ali Zaib</div>
                </td>
                <td align="right" valign="middle" style="padding-left:12px;">
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>REBIZ Sentinel X &mdash; Security Alert</title>
</head>
  ...
  <tr>
    ...
    def _get_install_path(self, agent_id, pkg_name):
        ...
        try:
            ...
            wazuh    = WazuhAPI(server_key=self.server_key)
    ...
    @staticmethod
    def _recommended_action_html(status_code, status_label=None):
        """Return guidance based on the normalized Defender status."""
        guidance = {
            'ACTION_REQUIRED': ('<strong>&#9888; Immediate Investigation Required.</strong> '
            ...
            'PENDING':     ('<strong>&#9203; Action Pending.</strong> '
                            'Defender has not yet confirmed completion. Continue monitoring the endpoint '
                            'and verify the final remediation result.'),
            'FAILED':      ('<strong>&#9888; Remediation Failed.</strong> '
                            'Review the Defender error details and remediate the threat on the endpoint.'),
            'COMPLETED':   ('<strong>&#10003; No Further Action Required.</strong> '
                            'Microsoft Defender successfully processed the detected threat. '
                            'Retain this alert as security/compliance evidence.'),
        }
        text = guidance.get(status_code, guidance['ACTION_REQUIRED'])
        bg   = '#fff7ed' if status_code in ('ACTION_REQUIRED', 'PENDING', 'FAILED') else '#f0fdf4'
        border = '#fed7aa' if status_code in ('ACTION_REQUIRED', 'PENDING', 'FAILED') else '#bbf7d0'
        fg   = '#9a3412' if status_code in ('ACTION_REQUIRED', 'PENDING', 'FAILED') else '#065f46'
    ...
    def _build_defender_email_body(self, alerts, period_start, period_end):
        ...
        for alert in alerts:
            ...
            # ── Threat fields ──────────────────────────────────────────────────
            threat_name  = defender_status['threat_name'] or _pick(
                eventd, 'threat name', 'threatName', 'Threat Name',
                'name', 'detection', 'Detection',
            )
            action       = defender_status['action_name'] or _pick(
                eventd, 'action name', 'actionName', 'Action Name',
                'action', 'Action',
            )
            execution_name = defender_status['execution_name']
            ...
            # The shared event interpreter is authoritative. The legacy
            # keyword helper remains only as a fallback for old events that
            # contain no structured Defender action fields.
            if defender_status['action_status'] != 'ACTION_REQUIRED' or (
                not action and not execution_name and not defender_status['error_code']
            ):
                status_code = defender_status['action_status']
                status_label = defender_status['action_status_label']
            else:
                status_code = 'ACTION_REQUIRED'
                status_label = 'Action Required'
            is_resolved = status_code == 'COMPLETED'
            status_color = {
                'COMPLETED': '#16803c',
                'PENDING': '#a16207',
                'FAILED': '#b91c1c',
                'ACTION_REQUIRED': '#c0392b',
            }.get(status_code, '#c0392b')
            ...
            if status_code == 'ACTION_REQUIRED':
                ...
                card_border, card_hdr_bg, card_hdr_fg = '#eab308', '#fefce8', '#854d0e'
            elif status_code == 'FAILED':
                card_border, card_hdr_bg, card_hdr_fg = '#ef4444', '#fef2f2', '#991b1b'
            elif defender_status['action_type'].lower() in ('quarantined', 'blocked'):
            ...
            # ── Recommended action ─────────────────────────────────────────────
            rec_action_td = self._recommended_action_html(status_code, status_label)
              ...
              <td style="padding:0 28px 20px;">
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"
                  ...
                  <tr><td style="padding:4px 16px 12px;">
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0"
                           style="font-size:11px;">
                      {_field_row('Action Status', status_label)}
                      {_field_row('Action Taken', 'Yes' if defender_status['action_taken'] else 'No')}
                      {_field_row('Action Type', defender_status['action_type'])}
                      {_field_row('Additional Action Required', 'Yes' if defender_status['additional_action_required'] else 'No')}
                      {_field_row('Execution Status', defender_status['execution_status'])}
                      {_field_row('Execution Result', defender_status['execution_result'])}
                      {_field_row('Error Code', defender_status['error_code'], mono=True, color='#b91c1c')}
                      {_field_row('Error Description', defender_status['error_description'])}
                      {_field_row('Additional Actions', defender_status['additional_actions'])}
                      {_field_row('IT Action Required', defender_status['notification_message'])}
                    </table>
                  </td></tr>
        ...
        for alert in alerts:
            ...
            win = data.get('win', {}) or {}
            eventd = win.get('eventdata', {}) or {}
            if normalize_defender_event(
                eventd,
                (source.get('rule', {}) or {}).get('description', ''),
    ...
    def _defender_subject(self, alerts):
        ...
        for a in alerts[:3]:
            ...
            normalized = normalize_defender_event(eventd, desc)
            statuses.append(normalized['action_status'])
        ...
        primary_status  = statuses[0] if statuses else 'ACTION_REQUIRED'
        ...
        status_phrase = {
            'COMPLETED':       'Threat Remediated',
            'PENDING':         'Threat Action Pending',
            'FAILED':          'Threat Remediation Failed',
            'ACTION_REQUIRED': 'Threat Action Required',
        }.get(primary_status, 'Threat Action Required')

        return (
            f'[{sev.upper()}] Wazuh Alert - {self.site_name} | '
            f'{status_phrase}: {primary_threat} | {primary_agent}{extra}'
        )
    ...
    def send_alert_email(self, recipient, subject, message, attachments=None):
        ...
        try:
            ...
            # Create message
            msg = MIMEMultipart()
            msg['From'] = f"REBIZ Sentinel X <{self.smtp_username}>"
    ...
    def send_fim_alert(self, alert_config):
        ...
        try:
            ...
            fim_levels = [
                int(event.get('source', {}).get('rule', {}).get('level', 0) or 0)
                for event in fim_data.get('results', [])
            ]
            fim_max_level = max(fim_levels) if fim_levels else 0
            fim_severity = (
                'CRITICAL' if fim_max_level >= 15 else
                'HIGH' if fim_max_level >= 12 else
                'MEDIUM' if fim_max_level >= 7 else 'LOW'
            )
            subject = (
                f"[{fim_severity}] Wazuh FIM Alert - {self.site_name}: "
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#1e0a3c 0%,#4f46e5 100%);
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
              <tr>
                <td>
                  <div style="color:#fff;font-weight:700;font-size:12px;">REBIZ Sentinel X — FIM Monitor</div>
    ...
    def send_severity_alert(self, alert_config, alerts_data=None):
        ...
        try:
            ...
            if alerts_data.get('manual_test', False) and not alerts_data.get('results', []):
                ...
                subject = (
                    f"[TEST] Wazuh Alert - {self.site_name} — "
                    f"{alert_config.name if hasattr(alert_config, 'name') else 'Alert Config'}"
                )
    ...
    <table role="presentation" width="580" cellspacing="0" cellpadding="0" border="0" style="max-width:580px;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 4px 20px rgba(13,27,75,0.15);">
      ...
      <tr>
        <td style="background:linear-gradient(135deg,#0d1b4b 0%,#1a3a8f 100%);padding:16px 30px;border-radius:0 0 10px 10px;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0">
            <tr>
              <td><div style="color:#fff;font-weight:700;font-size:12px;">REBIZ Sentinel X</div>
            ...
            if defender_alerts and not other_alerts:
                ...
                subject = (f"[{highest.upper()}] Wazuh Alert - {self.site_name} — {total_alerts} Alert(s) "
                ...
                highest = ('Critical' if alert_counts.get('critical', 0) else
                           ...
                           'Medium'   if alert_counts.get('medium',   0) else 'Low')
                subject = (
                    f"[{highest.upper()}] Wazuh Alert - {self.site_name} — "
                    f"{total_alerts} Security Alert(s) Detected"
                )
                body = self._build_html_email_body(
                    total_alerts=total_alerts,
                    alert_counts=alert_counts,
