"""
Threat Intelligence Service for REBIZ Sentinel X
...
def fetch_wazuh_cti(limit=200) -> list:
    """
    ...
    Bucket budget: limit * (1 + 1 + agent_size + 1 + agent_size) must stay
    well under OpenSearch's search.max_buckets limit (default 65535).
    With limit=200 and agent_size=20: 200 * 43 = 8,600 — safe.
    """
    items = []
    try:
        ...
        # Aggregation separates Active vs Solved per CVE to avoid mixing statuses.
        # Agent IPs/locations are enriched later via the inventory DB in the correlator,
        # so we only need a lightweight terms agg here (no nested top_hits).
        # agent_size kept small (20) so bucket count stays under max_buckets=65535.
        _agent_size = 20
        _agent_sub_agg = {"terms": {"field": "agent.name", "size": _agent_size}}
                ...
                "must": [
                    {
                        "bool": {
                            "should": [
                                ...
                                {"term": {"rule.groups": "Vulnerability Detector"}},
                                {"range": {"rule.id": {"gte": 23500, "lte": 23600}}},
                            ],
                    ...
                    },
                    {
                        "bool": {
                            "should": [
                                ...
                                {"exists": {"field": "data.vulnerability.id"}},
                            ],
                            "minimum_should_match": 1,
                        }
                    },
        ...
        def _build_agg_query(cve_size, ag_size):
            return {
                "size": 0,
                "query": _vuln_filter_query,
                "aggs": {
                    "unique_cves": {
                        "terms": {
                            "field": "data.vulnerability.cve",
                            "size": cve_size,
                            # Order by most-recently-detected timestamp so the
                            # newest CVEs always fill the bucket limit first.
                            # Using _count would cause old high-frequency CVEs to
                            # crowd out freshly-detected ones.
                            "order": {"max_date": "desc"},
                            "collect_mode": "breadth_first",
                        },
                        "aggs": {
                            # max_date drives the outer sort — must be declared
                            # before it is referenced in "order" above.
                            ...
                            "active_agents": {
                                "filter": {"terms": {"data.vulnerability.status": ["Active", "active"]}},
                                "aggs": {"by_agent": {"terms": {"field": "agent.name", "size": ag_size}}},
                            },
                            "solved_agents": {
                                "filter": {"terms": {"data.vulnerability.status": ["Solved", "solved"]}},
                                "aggs": {"by_agent": {"terms": {"field": "agent.name", "size": ag_size}}},
                            },
                        },
                    }
        ...
        agg_query = _build_agg_query(limit, _agent_size)

        def _extract_agent_list(agg_bucket: dict) -> list:
            """Pull {name} from an active/solved sub-aggregation bucket."""
            return [
                {'name': ab['key'], 'ip': '', 'location': ''}
                for ab in agg_bucket.get('by_agent', {}).get('buckets', [])
                if ab.get('key')
            ]

        # Execute with automatic retry at reduced sizes if OpenSearch returns a
        # too_many_buckets_exception (HTTP 503 / status 503).
        resp = None
        retry_sizes = [(limit, _agent_size), (100, 15), (50, 10)]
        for attempt_cve_size, attempt_ag_size in retry_sizes:
            try:
                query_to_run = _build_agg_query(attempt_cve_size, attempt_ag_size)
                ...
                logger.info(
                    f"Wazuh CTI: aggregation succeeded with cve_size={attempt_cve_size}, "
                    f"agent_size={attempt_ag_size}"
                )
                break
            except Exception as retry_exc:
                err_str = str(retry_exc)
                if 'too_many_buckets' in err_str or '503' in err_str:
                    logger.warning(
                        f"Wazuh CTI: too_many_buckets with cve_size={attempt_cve_size}, "
                        f"agent_size={attempt_ag_size}. Retrying smaller..."
                    )
                    continue
                raise

        if resp is None:
            logger.error("Wazuh CTI: all aggregation attempts failed due to bucket limits.")
            return items

        total_hits = resp.get('hits', {}).get('total', {}).get('value', 0)
        buckets = resp.get('aggregations', {}).get('unique_cves', {}).get('buckets', [])
        logger.info(
            f"Wazuh CTI: aggregation matched {total_hits} raw alerts, "
            f"produced {len(buckets)} unique CVE buckets"
        )
        if total_hits == 0:
            logger.warning(
                "Wazuh CTI: zero vulnerability alerts found. "
                "Ensure the Wazuh Vulnerability Detector module is enabled and has run at least once. "
                "Expected rule.id range 23500–23600 or rule.groups 'vulnerability-detector' "
                "with field data.vulnerability.cve present."
            )
        ...
        for bucket in buckets:
            ...
            # ── Active / Solved agent lists ─────────────────────────────────
            active_agents = _extract_agent_list(bucket.get('active_agents', {}))
            solved_agents = _extract_agent_list(bucket.get('solved_agents', {}))

            # A CVE is confirmed present (exposed) only if ACTIVE on ≥1 agent
            vuln_status = 'Active' if active_agents else 'Solved'
            ...
            items.append({
                ...
                'published_at': published,
                'severity': severity,
                'has_patch': (vuln_status == 'Solved') or _detect_patch(description),
...
def store_new_items(app, raw_items: list) -> list:
    ...
    For wazuh_cti items (sourced from the live Wazuh Vulnerability Detector)
    we do a true upsert: existing records are refreshed with the latest
    description (which contains updated active/solved agent lists), severity,
    patch status, and fetched_at timestamp.  This ensures a sync always
    reflects the current state of the environment rather than the first time
    each CVE was seen.

    All other sources are insert-only (skip existing GUIDs) to avoid
    overwriting AI analysis or manually-curated data.
    ...
    with app.app_context():
        for raw in raw_items:
            ...
            if not guid:
                continue
            existing = ThreatIntelItem.query.filter_by(guid=guid).first()
            if existing:
                if raw.get('source') == 'wazuh_cti':
                    # Refresh live Wazuh data so agent lists stay current
                    existing.description = raw.get('description', existing.description)
                    existing.severity = raw.get('severity', existing.severity)
                    existing.has_patch = raw.get('has_patch', existing.has_patch)
                    existing.has_mitigation = raw.get('has_mitigation', existing.has_mitigation)
        ...
        db.session.commit()
    logger.info(
        f"Stored {len(new_items)} new threat intel items, "
        f"refreshed {updated_count} existing Wazuh CTI items"
    )
    return new_items
...
def send_threat_email(item, analysis: dict, recipient: str) -> bool:
    ...
    try:
        ...
        msg['From'] = f"REBIZ Sentinel X <{Config.SMTP_USERNAME}>"
...
"""
    ...
    <div style="max-width:760px;margin:auto;background:#161b22;border-radius:8px;padding:24px;border:1px solid #30363d">
      <h2 style="color:#f0f6fc;border-bottom:1px solid #30363d;padding-bottom:12px">
        ⚠️ Threat Intelligence Alert — REBIZ Sentinel X
      </h2>
      <div style="background:{sev_colour}22;border-left:4px solid {sev_colour};padding:12px;border-radius:4px;margin-bottom:16px">
        <strong style="color:{sev_colour};text-transform:uppercase">{item.severity}</strong>
        &nbsp;{patch_badge}&nbsp;{mitigation_badge}
