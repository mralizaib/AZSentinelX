...
class HybridDVRConfigurator:
    ...
    def _get_isapi(self):
        if self._isapi is None:
            ...
            self._isapi = DVRClient(
                ...
                admin_verification_password=self.params.get('admin_verification_password') or None,
            )
        return self._isapi
    ...
    def _isapi_main_stream(self, channel_id: str):
        client = self._get_isapi()
        result = client.configure_main_stream(channel_id)
        ok = result.get('ok', False)
        if ok:
            res = result.get('applied_resolution', '?')
            br  = result.get('applied_bitrate', '?')
            detail = f"Main stream {res} H.264 CBR {br}kbps configured"
            if result.get('note'):
                detail += f" ({result['note']})"
        else:
            detail = result.get('note', 'Main stream ISAPI failed')
        return ok, detail
    ...
    def _isapi_create_user(self, username: str, password: str, role: str):
        ...
        try:
            ...
            err_msg = str(e)
            # Classify the failure so the report shows an actionable message
            # rather than a raw exception string.
            err_lower = err_msg.lower()
            if any(kw in err_lower for kw in (
                "double verif", "doubleverif", "usercheck", "illegal login",
                "secondary verif", "verification required",
            )):
                # Already enriched by create_user() — pass through as-is
                detail = err_msg
            elif "illegal login protection" in err_lower or "lock" in err_lower:
                detail = err_msg
            elif "401" in err_msg or "unauthorized" in err_lower:
                detail = (
                    f"Authentication failed creating user '{username}' (HTTP 401). "
                    f"The device may have triggered Illegal Login Protection due to "
                    f"repeated failed attempts. Check credentials and device security settings."
                )
            elif "403" in err_msg or "forbidden" in err_lower:
                detail = (
                    f"Permission denied creating user '{username}' (HTTP 403). "
                    f"The account used to connect may not have sufficient privileges "
                    f"to create users on this device."
                )
            else:
                detail = f"User '{username}' creation failed: {err_msg}"
            logger.error(f"[Hybrid] User creation failed for '{username}': {detail}")
            return False, detail
