...
def _get_logo_base64():
    """Load the REBIZ logo and return as a base64 data URI."""
    try:
        logo_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'static', 'img', 'rebiz-logo.png')
...
class ReportGenerator:
    def __init__(self, server_key=None):
        self.opensearch = OpenSearchAPI(server_key=server_key)
        self.server_key = self.opensearch.server_key
        self.server_timezone = get_server_timezone(self.server_key)
        self.server_timezone_label = timezone_label(self.server_key)
    ...
    def generate_report(self, report_config, start_time=None, end_time=None,
        ...
        # Convert timestamps to the originating server's local time for display.
        now_local = datetime.datetime.now(self.server_timezone)
        start_local = localize_iso_range(start_time, self.server_key)
        end_local = localize_iso_range(end_time, self.server_key)
        try:
            start_display = start_local.strftime('%Y-%m-%d %H:%M:%S %Z')
            end_display = end_local.strftime('%Y-%m-%d %H:%M:%S %Z')
        except Exception:
            start_display = start_time
            end_display = end_time

        # Keep the original alert timestamp and add a localized display value.
        localized_alerts = []
        for alert in alerts_data.get('results', []):
            alert_copy = alert.copy()
            src = dict(alert_copy.get('source', {}) or {})
            alert_copy['source'] = src
            if '@timestamp' in src:
                src['@timestamp_display'] = format_timestamp_for_server(
                    src['@timestamp'], self.server_key
                )
            ...
            localized_alerts.append(alert_copy)

        # Determine download metadata
        if downloaded_at is None:
            downloaded_at = now_local.strftime('%Y-%m-%d %H:%M:%S %Z')
        ...
        report_data = {
            'title': f"Security Alert Report — {now_local.strftime('%Y-%m-%d')} {self.server_timezone_label}",
            'generated_at': now_local.strftime('%Y-%m-%d %H:%M:%S %Z'),
            'downloaded_by': downloaded_by,
            'downloaded_at': downloaded_at,
            'period': {'start': start_display, 'end': end_display},
            'alerts': localized_alerts,
            'alert_counts': alert_counts,
            'severity_bar': severity_bar,
            'severity_levels': severity_levels,
            'total_alerts': alerts_data.get('total', 0),
            'timezone_note': f'All timestamps are in {self.server_timezone_label}',
