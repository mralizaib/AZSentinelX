/**
 * Reports JavaScript - REBIZ Sentinel X
...
function setLoadingMessage(title, message) {
    ...
    const msgEl = document.getElementById('loading-modal-message');
...
function generateReport(reportId) {
    setLoadingMessage('Generating Report…', 'Fetching alerts and building your report. PDF generation may take up to 60 seconds — please keep this window open.');

    const loadingModal = new bootstrap.Modal(document.getElementById('loading-modal'));
    loadingModal.show();

    // Use AbortController with a 3-minute timeout
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 180000);

    fetch(`/api/reports/${reportId}/generate`, { signal: controller.signal })
        .then(response => {
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                return response.blob().then(blob => {
                    ...
                    a.href = url;
                    a.download = `rebiz_sentinel_x_report_${new Date().toISOString().replace(/[:.]/g, '-')}.pdf`;
        ...
        .catch(error => {
            ...
            if (errorMessage) {
                errorMessage.textContent = error.name === 'AbortError'
                    ? 'Report generation timed out. Try using HTML format instead of PDF, or reduce the time range.'
...
function sendReport(reportId) {
    ...
    setLoadingMessage('Sending Report…', 'Generating the report and emailing it to the recipients. This may take up to 60 seconds.');
...
/**
 * Handle generating a custom report (Quick Report section)
 */
function handleGenerateReport() {
    const severityLevels = [];
    document.querySelectorAll('input[name="generate-severity-level"]:checked').forEach(cb => severityLevels.push(cb.value));
    ...
    setLoadingMessage(
        isPdf ? 'Generating PDF Report…' : 'Generating HTML Report…',
        isPdf
            ? 'Fetching alerts and building your PDF. This may take up to 60 seconds for large datasets — please keep this window open.'
            : 'Fetching alerts and building your HTML report…'
    );
    ...
    fetch('/api/reports/generate', {
        ...
        body: JSON.stringify({ severity_levels: severityLevels, time_range: timeRange, format: format }),
            ...
            if (contentType && contentType.includes('application/json')) {
                return response.json().then(data => {
                    ...
                    if (data.html && preview) {
                        ...
                        a.style.display = 'none';
                        a.href = url;
                        a.download = `rebiz_sentinel_x_report_${new Date().toISOString().replace(/[:.]/g, '-')}.html`;
                        document.body.appendChild(a);
                        a.click();
                        window.URL.revokeObjectURL(url);
                    }
        ...
        .catch(error => {
            ...
            if (errorMessage) {
                errorMessage.textContent = error.name === 'AbortError'
                    ? 'Report generation timed out. Try using HTML format instead of PDF, or select a shorter time range.'
                    : `Error generating report: ${error.message}`;
            }
            errorModal.show();
        });
