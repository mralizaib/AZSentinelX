"""
Threat Intelligence Correlation Engine for REBIZ Sentinel X
...
NETWORK_KEYWORDS = [
    ...
    'catalyst', 'ios xe', 'cisco ios', 'smart switch', 'nexus', 'cisco switch',
]

# Placeholder entries for network devices that are present in the environment
# but do not have Wazuh agents (e.g. pfSense firewalls, Cisco smart switches).
# When a threat matches NETWORK_KEYWORDS but no agent name matches, these are
# surfaced so the analyst knows their network gear may be affected.
NETWORK_INFRA_PLACEHOLDERS = [
    {
        'keywords': ['pfsense', 'pf sense', 'opnsense', 'opn sense'],
        'name': 'pfSense Firewall',
        'role': 'Network Firewall',
    },
    {
        'keywords': ['cisco', 'catalyst', 'ios xe', 'cisco ios', 'smart switch', 'nexus', 'cisco switch'],
        'name': 'Cisco Smart Switch / Router',
        'role': 'Network Switch / Router',
    },
]
...
def match_agents_to_threat(item, agents: list) -> list:
    ...
    for agent in agents:
        ...
        # --- Network device match (agent-name based, word-boundary safe) ---
        if is_network:
            for kw in NETWORK_KEYWORDS:
                # Use word-boundary regex so short keywords like 'asa', 'vpn',
                # 'switch' don't match arbitrary substrings inside agent names
                # (e.g. 'asa' must not match 'IqraSami' or 'AsadAli').
                if re.search(r'\b' + re.escape(kw) + r'\b', agent_name_lower):
                    reasons.append(
    ...
    # Network devices (pfSense, Cisco switches) don't run Wazuh agents, so they
    # never appear in the inventory loop above.  When the threat text mentions
    # keywords specific to those device types, add a placeholder entry so the
    # analyst is warned that their network gear may be affected.
    if is_network:
        agent_matched_ids = {a['id'] for a in affected}
        for placeholder in NETWORK_INFRA_PLACEHOLDERS:
            matched_kws = [kw for kw in placeholder['keywords'] if kw in combined]
            if matched_kws:
                placeholder_id = f"__network__{placeholder['name'].replace(' ', '_').lower()}"
                if placeholder_id not in agent_matched_ids:
                    agent_matched_ids.add(placeholder_id)
                    affected.append({
                        'id': placeholder_id,
                        ...
                        'reasons': [
                            f"Threat mentions {', '.join(matched_kws[:3])} — your environment "
                            f"includes {placeholder['name']} devices that may be affected. "
                            f"Check the device firmware/software version against this CVE."
                        ],
                        'match_source': 'network_infra',
                    })
...
def correlate_threat_item(item, agents: list) -> dict:
    ...
    if is_wazuh_cti:
        ...
        # Wazuh's Vulnerability Detector only flags a CVE on agents where it
        # found and version-matched the affected package.  Agents that don't
        # have the software inventoried (or whose scanner hasn't run recently)
        # are silently absent.  We surface these as "potential risk" agents so
        # the analyst knows to verify them manually.
        #
        # Determine target platform from the CVE title / description.
        combined_text = f"{getattr(item, 'title', '')} {desc}".lower()
        _target_win  = any(kw in combined_text for kw in WINDOWS_KEYWORDS)
        _target_lin  = any(kw in combined_text for kw in LINUX_KEYWORDS)
        if not _target_win and not _target_lin:
            _target_win = True   # default: flag Windows (largest fleet)

        seen_lower = {n.lower() for n in seen}
        potential_risk_agents = []
        for inv_agent in agents:
            a_name = inv_agent.get('name', '')
            if a_name.lower() in seen_lower:
                continue  # already in confirmed list
            platform = inv_agent.get('os_platform', '').lower()
            is_win = 'windows' in platform
            is_lin = 'linux' in platform
            if (_target_win and is_win) or (_target_lin and is_lin):
                potential_risk_agents.append({
                    'id':       inv_agent.get('id'),
                    'name':     a_name,
                    'ip':       inv_agent.get('ip', ''),
                    'os':       f"{inv_agent.get('os_name','')} {inv_agent.get('os_version','')}".strip(),
                    'location': inv_agent.get('location', ''),
        ...
        return {
            ...
            'potential_risk_agents': potential_risk_agents,
            'alert_matches': [],
        ...
        with app.app_context():
            ...
            for item in items:
                try:
                    ...
                    corr.potential_risk_agents = json.dumps(result.get('potential_risk_agents', []))
...
def correlate_single_item(app, item_id: int) -> dict:
    ...
    with app.app_context():
        ...
        return {
            ...
            'potential_risk_agents': result.get('potential_risk_agents', []),
        }
