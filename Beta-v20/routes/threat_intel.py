...
@threat_intel_bp.route('/api/threat-intel/vuln-severity-detail')
@login_required
def vuln_severity_detail():
    """
    Return paginated live vulnerability records from OpenSearch filtered by
    severity (critical/high/medium/low) or status (pending).
    Query params:
        filter  – 'critical' | 'high' | 'medium' | 'low' | 'pending'
    ...
    try:
        ...
        api = OpenSearchAPI()
        if not api.client:
            return jsonify({'error': 'OpenSearch not connected'}), 503

        flt  = request.args.get('filter', 'critical').lower()
        page = max(1, int(request.args.get('page', 1)))
        size = min(100, max(1, int(request.args.get('size', 25))))

        _PENDING_RAW = ['pending', 'pending confirmation']
        _ACTIVE_RAW  = ['active', 'still applicable', 'unpatched']

        # Build severity / status filter
        if flt == 'pending':
            status_clauses = [{"term": {"data.vulnerability.status": s}} for s in _PENDING_RAW]
            extra_filter = {"bool": {"should": status_clauses, "minimum_should_match": 1}}
        else:
            extra_filter = {"term": {"data.vulnerability.severity": flt.capitalize()}}
        ...
        resp = api.client.search(
            index="wazuh-alerts-*", body=query_body, request_timeout=45
        )
        ...
        logger.error(f"Vuln severity detail error: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500


@threat_intel_bp.route('/api/threat-intel/vuln-infra-dashboard')
@login_required
def vuln_infra_dashboard():
    """
    Return aggregated vulnerability statistics from Wazuh/OpenSearch for the
    'Vulnerabilities in Infrastructure' dashboard panel on the Threat Intel page.
    All aggregations run in a single OpenSearch query for efficiency.
    """
    try:
        from opensearch_api import OpenSearchAPI
        api = OpenSearchAPI()
        ...
        agg_query = {
            ...
            "aggs": {
                ...
                "by_status": {
                    "terms": {"field": "data.vulnerability.status", "size": 20}
                },
                "top_cves": {
                    "terms": {
        ...
        resp = api.client.search(
            index="wazuh-alerts-*", body=agg_query, request_timeout=60
        )
        ...
        # ── Pending / Active counts via status field ───────────────────────
        _PENDING_RAW = {"pending", "pending confirmation"}
        _ACTIVE_RAW  = {"active", "still applicable", "unpatched"}
        pending_count = 0
        active_count  = 0
        for b in aggs.get("by_status", {}).get("buckets", []):
            n = b["key"].lower().strip()
            if n in _PENDING_RAW:
                pending_count += b["doc_count"]
            elif n in _ACTIVE_RAW:
                active_count += b["doc_count"]
        ...
        # ── CVSS score distribution (bar chart data) ──────────────────────
        # data.vulnerability.score.base is a keyword in this Wazuh mapping,
        # so we use terms agg and then group raw string values into bands.
        _cvss_bands = {
            "0.0": 0, "0.1–3.9": 0, "4.0–4.9": 0,
            "5.0–5.9": 0, "6.0–6.9": 0, "7.0–7.9": 0,
            "8.0–8.9": 0, "9.0–10.0": 0,
        }
        for b in aggs.get("cvss_terms", {}).get("buckets", []):
            try:
                v = float(b["key"])
            except (ValueError, TypeError):
                continue
            ...
            if v == 0.0:
                ...
                _cvss_bands["0.1–3.9"] += cnt
            elif v < 5.0:
                _cvss_bands["4.0–4.9"] += cnt
            elif v < 6.0:
                _cvss_bands["5.0–5.9"] += cnt
            elif v < 7.0:
                _cvss_bands["6.0–6.9"] += cnt
            elif v < 8.0:
                _cvss_bands["7.0–7.9"] += cnt
            elif v < 9.0:
                _cvss_bands["8.0–8.9"] += cnt
            else:
                _cvss_bands["9.0–10.0"] += cnt
        cvss_scores = [k for k, c in _cvss_bands.items() if c > 0]
        cvss_counts = [_cvss_bands[k] for k in cvss_scores]

        # ── By year × severity (grouped bar chart) ────────────────────────
        # Run as a separate query so a field-type mismatch doesn't break the
        # main stats.  Falls back to empty arrays if the field is a keyword.
        year_labels = []
        year_critical, year_high, year_medium, year_low = [], [], [], []
        try:
            year_resp = api.client.search(
                index="wazuh-alerts-*",
                body={
                    "size": 0,
                    "query": vuln_filter,
            ...
            for b in year_resp.get("aggregations", {}).get("by_year_severity", {}).get("buckets", []):
                year_labels.append(b["key_as_string"])
                sev_sub = {
                    sb["key"].lower(): sb["doc_count"]
                    for sb in b.get("by_severity", {}).get("buckets", [])
                }
                year_critical.append(sev_sub.get("critical", 0))
                year_high.append(sev_sub.get("high", 0))
                year_medium.append(sev_sub.get("medium", 0))
                year_low.append(sev_sub.get("low", 0))
        except Exception as year_err:
            logger.warning(f"Vuln year-chart aggregation skipped: {year_err}")

        return jsonify({
            "severity_counts": sev_counts,
            "pending_count": pending_count,
            "active_count": active_count,
            "top_cves": top5("top_cves"),
            "top_agents": top5("top_agents"),
            "top_packages": top5("top_packages"),
            "cvss_labels": cvss_scores,
            "cvss_counts": cvss_counts,
            "year_labels": year_labels,
            "year_critical": year_critical,
            "year_high": year_high,
            "year_medium": year_medium,
            "year_low": year_low,
        })

    except Exception as e:
        logger.error(f"Vuln infra dashboard error: {e}", exc_info=True)
        return jsonify({"error": str(e)}), 500
