            ...
            if results and 'error' not in results:
                ...
                for alert in alerts:
                    try:
                        ...
                        # ── Noise suppression pre-screen ──────────────────────
                        # Skip alerts that match active noise filters (e.g. rule 4804,
                        # level < 7, etc.) to avoid filling the DB with low-value events.
                        try:
                            from log_filter_engine import should_store_alert
                            if not should_store_alert(source):
                                skipped_count += 1
                                continue
                        except Exception:
                            pass  # If filter engine fails, store the alert anyway
                        # ─────────────────────────────────────────────────────
    ...
    with app.app_context():
        try:
            ...
            for alert_config in alert_configs:
                try:
                    # Fetch real-time alerts from OpenSearch based on criteria
                    ...
                    try:
                        ...
                        if is_high_critical:
                            opensearch_api_obj = OpenSearchAPI()
                            ...
                            if recent_results and 'results' in recent_results:
                                for alert in recent_results['results']:
                                    send_alert_to_integrations(alert.get('_source', {}))
...
def run_itdr_scan():
    """Scheduled job: run ITDR/XDR detection scan every 5 minutes."""
    if not scheduler or not scheduler.app:
        logger.error("Scheduler app not initialized for itdr_scan")
        return
    try:
        from itdr_engine import run_itdr_scan as _scan
        _scan(scheduler.app)
    except Exception as e:
        logger.error(f"Error in scheduled ITDR scan: {e}", exc_info=True)
...
def update_scheduler_jobs():
    ...
    try:
        ...
        with scheduler.app.app_context():
            ...
            # Add active threat scan job — runs every 2 minutes
            # Scans for level>=12 alerts buried in noise, fires immediate email
            try:
                scheduler.remove_job('active_threat_scan')
            except Exception:
                pass
            scheduler.add_job(
                id='active_threat_scan',
                func=run_active_threat_scan,
                trigger='interval',
                minutes=2,
                replace_existing=True,
                max_instances=1
...
def init_app(app):
    ...
    with app.app_context():
        ...
        # ITDR/XDR scan - runs every 5 minutes
        scheduler.add_job(
            func=run_itdr_scan,
            trigger="interval",
            minutes=5,
        ...
        # Active Threat Scan — runs every 2 minutes
        # Detects level>=12 alerts buried in noise and fires immediate email to all recipients
        scheduler.add_job(
            func=run_active_threat_scan,
            trigger="interval",
            minutes=2,
