...
def correlate_threat_item(item, agents: list) -> dict:
    ...
    if is_wazuh_cti:
        ...
        # fetch_wazuh_cti embeds three separate structured agent lists:
        #   [WAZUH_ACTIVE_AGENTS:...]   = agents where the CVE is still unpatched
        #   [WAZUH_PENDING_AGENTS:...]  = agents awaiting scan or status confirmation
        ...
        raw_pending = _parse_wazuh_agent_list(desc, 'WAZUH_PENDING_AGENTS')
        ...
        if not raw_active and not raw_pending and not raw_solved:
        ...
        def _build_agent_entry(raw, vuln_status_flag):
            ...
            Passes through all structured vulnerability fields captured per-agent
            by fetch_wazuh_cti so the UI can display the full data.vulnerability.*
            payload alongside agent.ip / agent.name.
            """
            ...
            if vuln_status_flag == 'Active':
                reason_text = f"Wazuh Vulnerability Detector — {cve_label} is UNPATCHED on this agent"
            elif vuln_status_flag == 'Pending':
                reason_text = f"Wazuh Vulnerability Detector — {cve_label} is PENDING scan/update on this agent"
            else:
                reason_text = f"Wazuh Vulnerability Detector — {cve_label} is PATCHED on this agent"
            entry = {
                ...
                'location': location,
                'vuln_status': vuln_status_flag,   # 'Active', 'Pending', or 'Solved'
                'reasons': [reason_text],
                'match_source': 'wazuh_vuln_detector',
            }
            # Pass through all structured vulnerability fields when present.
            # NOTE: 'vuln_status' is intentionally excluded — entry['vuln_status']
            # is already set to the normalized classification flag ('Active',
            # 'Pending', or 'Solved') above and must not be overwritten by the
            # raw Wazuh string (e.g. "solved", "fixed", "still applicable").
            vuln_fields = [
                'vuln_title', 'vuln_assigner', 'vuln_classification', 'vuln_cve',
                'vuln_severity', 'vuln_published',
                'cvss3_base_score', 'cvss3_attack_vector', 'cvss3_conf_impact',
                'score_base', 'package_name', 'vuln_rationale', 'vuln_reference',
                'scanner_reference',
            ]
        ...
        for raw in raw_pending:
            ...
            if n and n not in seen:
                seen.add(n)
                affected_agents.append(_build_agent_entry(raw, 'Pending'))
        ...
        # is_confirmed_present = True when at least one agent is Active OR Pending
        is_confirmed = any(a['vuln_status'] in ('Active', 'Pending') for a in affected_agents)
        active_count  = sum(1 for a in affected_agents if a['vuln_status'] == 'Active')
        pending_count = sum(1 for a in affected_agents if a['vuln_status'] == 'Pending')
        solved_count  = sum(1 for a in affected_agents if a['vuln_status'] == 'Solved')

        if active_count and pending_count:
            summary = (
                f"Wazuh Vulnerability Detector: {cve_label} is UNPATCHED on {active_count} agent(s), "
                f"PENDING on {pending_count} agent(s), and PATCHED on {solved_count} agent(s)."
            )
            action = (
                f"Apply the available patch to all {active_count} UNPATCHED agent(s) immediately. "
                f"Investigate the {pending_count} PENDING agent(s) to confirm their status."
            )
        elif active_count:
            summary = (
                f"Wazuh Vulnerability Detector: {cve_label} is UNPATCHED on "
                f"{active_count} agent(s) and PATCHED on {solved_count} agent(s)."
                if solved_count else
                f"Wazuh Vulnerability Detector: {cve_label} is UNPATCHED on {active_count} agent(s)."
            )
            action = f"Apply the available patch to all {active_count} UNPATCHED agent(s) immediately."
        elif pending_count:
            summary = (
                f"Wazuh Vulnerability Detector: {cve_label} is PENDING scan/update on "
                f"{pending_count} agent(s). {solved_count} agent(s) have already been patched."
                if solved_count else
                f"Wazuh Vulnerability Detector: {cve_label} is PENDING scan/update on {pending_count} agent(s)."
            )
            action = f"Investigate the {pending_count} PENDING agent(s) to trigger a re-scan or confirm status."
        else:
            summary = (
                f"Wazuh Vulnerability Detector: {cve_label} has been PATCHED on all "
                f"{solved_count} detected agent(s)."
            )
            action = "All detected agents have the patch applied. No immediate action required."

        return {
            'affected_agents': affected_agents,
            'affected_count': len(affected_agents),
            'alert_matches': [],
            'env_relevance_score': min(10, max(1, active_count + pending_count)),
            'env_summary': summary,
