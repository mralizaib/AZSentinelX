...
def _check_alerts_for_server(server_key, alert_configs):
    """
    ...
    if not scheduler or not scheduler.app:
        logger.error("Scheduler or Scheduler app is not initialized")
        return
    ...
    with app.app_context():
        try:
            ...
            email_alerts = EmailAlerts(server_key=server_key)
            logger.info(
                "Checking %d alert configuration(s) against Wazuh server '%s'",
                len(alert_configs),
                email_alerts.server_name,
            )

            from routes.integrations import send_alert_to_integrations
            # Process each alert configuration
            for alert_config in alert_configs:
                try:
                    # FIM configs have a completely separate processing path.
                    # They must ONLY fetch syscheck events for the configured
                    # agents/paths — never go through send_severity_alert.
                    if alert_config.is_fim():
                        logger.info(
                            f"🔍 Processing FIM alert config {alert_config.id} "
                            f"({alert_config.name}) → {alert_config.email_recipient}"
                        )
                        try:
                            result = email_alerts.send_fim_alert(alert_config)
                            # send_fim_alert returns (bool, str); unpack properly
                            ok   = result[0] if isinstance(result, tuple) else bool(result)
                            msg  = result[1] if isinstance(result, tuple) else ""
                            if ok and "No FIM" in msg:
                                logger.debug(
                                    f"⏭ FIM config ID {alert_config.id}: {msg}"
                                )
                            elif ok and "already sent" in msg.lower():
                                logger.debug(
                                    f"⏭ FIM config ID {alert_config.id}: {msg}"
                                )
                            elif ok:
                                logger.info(
                                    f"✅ FIM email dispatched for config ID {alert_config.id}"
                                )
                            else:
                                logger.error(
                                    f"❌ FIM alert failed for config ID {alert_config.id}: {msg}"
                                )
                        except Exception as fim_err:
                            logger.error(
                                f"❌ Exception sending FIM alert for config ID "
                                f"{alert_config.id}: {fim_err}"
                            )
                        continue  # skip severity logic below entirely

                    # ── Standard severity-based alert processing ──────────────
                    ...
                    try:
                        ...
                        if ok:
                            ...
                            msg = result[1] if isinstance(result, tuple) else ""
                            logger.error(f"❌ Failed to send alert for config ID {alert_config.id}: {msg}")
                    except Exception as send_error:
                        logger.error(f"❌ Exception while sending alert for config ID {alert_config.id}: {str(send_error)}")
                        result = False

                    # Integration hook: send to external SOAR/SIEM
                    try:
                        # Fetch some recent alerts to send to integrations if this is a high severity check
                        if is_high_critical:
                            opensearch_api_obj = OpenSearchAPI(server_key=server_key)
                            recent_results = opensearch_api_obj.search_alerts(
                                severity_levels=['high', 'critical'],
                                limit=5
                            )
                            if recent_results and 'results' in recent_results:
                                for alert in recent_results['results']:
                                     send_alert_to_integrations(
                                         alert.get('source') or alert.get('_source', {})
                                     )
                    except Exception as integ_error:
                        logger.error(f"Error sending to external integrations: {integ_error}")

                except Exception as e:
                    logger.error(f"Error processing alert config {alert_config.id}: {str(e)}")

        except Exception as e:
            logger.error(f"Error in check_alerts job: {str(e)}")
            import traceback
            logger.error(traceback.format_exc())


def check_alerts():
    """Check every enabled alert configuration against every configured server."""
    try:
        from app import app
        with app.app_context():
            ...
            configured_servers = Config.get_configured_servers()
            if not configured_servers:
                logger.error("No configured Wazuh/OpenSearch servers available for alert checks")
                return

            for profile in configured_servers:
                _check_alerts_for_server(profile['key'], alert_configs)
    except Exception as e:
        logger.error(f"Error preparing multi-server alert check: {e}", exc_info=True)


def check_and_send_alerts():
    """
    Wrapper for check_alerts to maintain compatibility
    """
    check_alerts()
...
def generate_and_send_reports():
    ...
    from app import app
    try:
        # Use the application context from the main app
        with app.app_context():
                ...
                try:
                    ...
                    try:
                        ...
                        <body>
                            ...
                            <p>This is an automated email from REBIZ Sentinel X.</p>
                        </body>
                        </html>
                        """
...
def run_threat_intel_refresh():
    """Scheduled job: fetch threat intel feeds, analyse, and send emails."""
    if not scheduler or not scheduler.app:
        logger.error("Scheduler app not initialized for threat_intel_refresh")
        return
    try:
        from threat_intel_service import run_full_refresh
        run_full_refresh(scheduler.app)
    except Exception as e:
        logger.error(f"Error in scheduled threat intel refresh: {e}", exc_info=True)


def run_compliance_evaluation():
    """Scheduled deterministic compliance evaluation for configured servers."""
    if not scheduler.app:
        logger.error("Scheduler app not initialized for compliance evaluation")
        return
    try:
        with scheduler.app.app_context():
            from compliance_engine import evaluate_wazuh
            configured = Config.get_configured_servers()
            if not configured:
                logger.info("No configured Wazuh/OpenSearch servers for compliance evaluation")
                return
            for profile in configured:
                result = evaluate_wazuh(profile['key'], hours=2, limit=1000)
                logger.info(
                    "Compliance evaluation complete server=%s events=%s created=%s updated=%s",
                    profile['key'], result['evaluated_events'],
                    result['findings_created'], result['findings_updated'],
                )
    except Exception as e:
        logger.error(f"Error in scheduled compliance evaluation: {e}", exc_info=True)
...
def update_scheduler_jobs():
    ...
    try:
        ...
        with scheduler.app.app_context():
            ...
            # Add threat intelligence refresh job — runs every 6 hours
            try:
                scheduler.remove_job('threat_intel_refresh')
            except Exception:
                pass
            scheduler.add_job(
                id='threat_intel_refresh',
                func=run_threat_intel_refresh,
                trigger='interval',
                hours=6,
                replace_existing=True,
                max_instances=1
            ...
            try:
                scheduler.remove_job('compliance_evaluation')
            except Exception:
                pass
            scheduler.add_job(
                id='compliance_evaluation',
                func=run_compliance_evaluation,
                trigger='interval',
                hours=1,
                replace_existing=True,
                max_instances=1,
