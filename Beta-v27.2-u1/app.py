...
# Session cookie settings. Authentication is deliberately browser-session-only:
# closing the browser removes the Flask session cookie, and no remember-me
# cookie is issued by the login flow.
app.config["PERMANENT_SESSION_LIFETIME"] = timedelta(minutes=10)
app.config["SESSION_COOKIE_PERMANENT"] = False
app.config["SESSION_REFRESH_EACH_REQUEST"] = False
app.config["SESSION_COOKIE_HTTPONLY"] = True
app.config["SESSION_COOKIE_SAMESITE"] = "Lax"
# Existing remember cookies are rejected by before_request unless they belong
# to the current browser session. New logins never issue one.
...
def _clear_auth_cookie(response):
    """Remove legacy Flask-Login remember cookies during forced logout."""
...
def _forced_login_redirect():
    """Return a login redirect that cannot preserve a remembered session."""
    return _clear_auth_cookie(
        redirect(url_for('auth.login', next=request.url))
    )


def _forced_login_json(message):
    """Return an API auth error while expiring any legacy remember cookie."""
    return _clear_auth_cookie(jsonify({
        'error': message,
        'redirect': '/login',
    }))
...
try:
    ...
    try:
        ...
        logger.info("Compliance & GRC blueprint registered successfully")
    except ImportError as e:
        logger.warning(f"Could not import compliance blueprint: {e}")
    ...
    try:
        from routes.memory_monitor import memory_monitor_bp
        app.register_blueprint(memory_monitor_bp)
        logger.info("Memory Monitor blueprint registered successfully")
    except ImportError as e:
        logger.warning(f"Could not import memory_monitor blueprint: {e}")

    try:
        from routes.dvr_hybrid_config import dvr_hybrid_bp
        app.register_blueprint(dvr_hybrid_bp)
        logger.info("DVR Hybrid Configuration blueprint registered successfully")
    except ImportError as e:
        logger.warning(f"Could not import dvr_hybrid blueprint: {e}")

    try:
        from routes.auditor_dashboard import auditor_dashboard_bp
        app.register_blueprint(auditor_dashboard_bp)
        logger.info("Auditor Dashboard blueprint registered successfully")
    except ImportError as e:
        logger.warning(f"Could not import auditor_dashboard blueprint: {e}")

    try:
        from routes.wazuh import wazuh_bp
        app.register_blueprint(wazuh_bp)
        logger.info("Wazuh server selector registered successfully")
    except ImportError as e:
        logger.warning(f"Could not import Wazuh server selector: {e}")
...
with app.app_context():
    try:
        from models import (User, AlertConfig, ReportConfig, AiInsightTemplate, AiInsightResult,
                            ...
                            ComplianceFramework, ComplianceDomain, ComplianceRequirement,
                            ComplianceControl, ComplianceRule, ComplianceRuleControl,
                            ComplianceResult, ComplianceFinding, ComplianceEvidence,
                            ComplianceRisk, ComplianceRemediation, ComplianceAuditLog,
                            ComplianceWorkbook, ComplianceWorkbookRecord)
        db.create_all()

        try:
            from compliance_engine import ensure_compliance_schema, ensure_default_catalog
            ensure_compliance_schema()
            ensure_default_catalog()
            logger.info("Compliance catalog initialized")
        except Exception as _compliance_seed_error:
            db.session.rollback()
            logger.warning(f"Compliance catalog initialization note: {_compliance_seed_error}")

        # Add MFA columns to databases created before MFA was introduced.
        # Quote the table name because "user" is reserved by PostgreSQL.
        try:
            ...
            for column_name, column_type in mfa_columns:
                if column_name not in user_columns:
                    db.session.execute(db.text(
                        f'ALTER TABLE "user" ADD COLUMN {column_name} {column_type}'
                    ))
                    db.session.commit()
        except Exception as _mfa_migration_error:
            db.session.rollback()
            logger.debug(f"MFA column migration note: {_mfa_migration_error}")
        ...
        # Migrate legacy roles to new DPCM role
        try:
            legacy_users = User.query.filter(
                User.role.in_(['dvr_technician', 'senior_dvr_technician'])
            ).all()
            if legacy_users:
                for _u in legacy_users:
                    logger.info(f"Migrating user '{_u.username}' role '{_u.role}' → 'dpcm'")
                    _u.role = 'dpcm'
                db.session.commit()
                logger.info(f"Migrated {len(legacy_users)} user(s) to dpcm role")
        except Exception as _role_err:
            db.session.rollback()
            logger.warning(f"Role migration note: {_role_err}")
...
def before_request():
    ...
    if current_user.is_authenticated:
        now = time.time()
        session_started = session.get('_auth_session_started')
        last_activity = session.get('_last_activity')

        # Flask-Login can restore a user from an old remember cookie even
        # though the browser's session cookie was closed. Reject that restored
        # authentication so browser reopening always requires password + MFA.
        if not session_started:
            logout_user()
            session.clear()
            if request.path.startswith('/api/'):
                return _forced_login_json('Authentication required'), 401
            return _forced_login_redirect()

        try:
            idle_seconds = now - float(last_activity)
        except (TypeError, ValueError):
            idle_seconds = SESSION_IDLE_TIMEOUT_SECONDS + 1

        if idle_seconds >= SESSION_IDLE_TIMEOUT_SECONDS:
            logout_user()
            session.clear()
            if request.path.startswith('/api/'):
                return _forced_login_json('Session expired due to inactivity'), 401
            return _forced_login_redirect()

        # Keep the session non-permanent and record activity on every valid
        # authenticated request. Browser-side keepalive requests are only sent
        # after real user interaction (see layout.html).
...
def inject_platform_context():
    """Expose safe branding and active Wazuh server metadata to templates."""
    from config import Config
    return {
        'wazuh_server_options': Config.get_server_options(),
        'active_wazuh_server': session.get('wazuh_server', 'primary'),
        'platform_name': 'REBIZ Sentinel X',
    }
...
def favicon():
    return redirect(url_for('static', filename='img/rebiz-logo.png'))
