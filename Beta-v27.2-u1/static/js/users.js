/**
 * Users JavaScript - REBIZ Sentinel X
...
function displayUsers(users, container) {
    ...
    users.forEach(user => {
        ...
        const mfaActive = user.mfa_status === 'Activated';
        const mfaPending = user.mfa_status === 'Pending setup';
        const mfaBadge = mfaActive
            ? '<span class="badge bg-success"><i class="fas fa-check me-1"></i>Activated</span>'
            : mfaPending
                ? '<span class="badge bg-warning text-dark"><i class="fas fa-clock me-1"></i>Pending setup</span>'
                : '<span class="badge bg-secondary"><i class="fas fa-minus me-1"></i>Disabled</span>';
        const mfaBtn = `<button class="btn ${mfaActive || mfaPending ? 'btn-outline-danger' : 'btn-outline-success'} btn-sm btn-mfa"
                    data-id="${user.id}" data-enabled="${user.mfa_enabled}" title="${mfaActive || mfaPending ? 'Disable MFA' : 'Enable MFA'}">
                    <i class="fas ${mfaActive || mfaPending ? 'fa-shield-halved' : 'fa-shield'}"></i>
               </button>`;
...
function addUserButtonListeners() {
    ...
    document.querySelectorAll('.btn-mfa').forEach(btn => {
        btn.addEventListener('click', () => toggleMfa(btn.dataset.id, btn.dataset.enabled === 'true'));
    });
}

function toggleMfa(userId, currentlyEnabled) {
    const action = currentlyEnabled ? 'disable' : 'enable';
    const warning = currentlyEnabled
        ? 'Disable MFA for this user? They will no longer need an authenticator code.'
        : 'Enable MFA for this user? They will scan a QR code and confirm it at their next login.';
    if (!confirm(warning)) return;

    fetch(`/api/users/${userId}/mfa`, {
        method: 'POST',
        ...
        .then(data => {
            loadUsers();
            showSuccess(data.message || `MFA ${action}d successfully.`);
        })
        .catch(err => showError(`Error updating MFA: ${err.message}`));
}
...
const ROLE_OPTIONS = [
    ...
    { value: 'auditor',     label: 'Auditor',        color: 'info' },
    { value: 'soc_analyst', label: 'SOC Analyst',    color: 'primary' },
    { value: 'dpcm',        label: 'DPCM',           color: 'warning text-dark' },
    { value: 'admin',       label: 'Administrator',  color: 'danger' },
];

const ROLE_HINTS = {
    viewer:      'Dashboard, Alerts, Reports',
    auditor:     'Auditor Dashboard (compliance view), Alerts, Reports — informational noise excluded from all statistics',
