...
MODULE_PERMISSIONS = {
    ...
    'dvr_hybrid':         'DVR/NVR Hybrid AutoDeploy',
...
class User(UserMixin, db.Model):
    ...
    # Local TOTP MFA. The secret is encrypted by the database/platform at rest
    # where supported; it is never returned by the users API.
    mfa_enabled = db.Column(db.Boolean, default=False, nullable=False)
    mfa_configured = db.Column(db.Boolean, default=False, nullable=False)
    ...
    ROLE_DEFAULT_PERMISSIONS = {
        ...
        # Auditors get their own clean compliance dashboard + read-only alerts + reports
        'auditor': {'dashboard', 'auditor_dashboard', 'alerts', 'reports', 'compliance'},
    }
...
class ComplianceFramework(db.Model):
    ...
    name = db.Column(db.String(200), nullable=False)
    description = db.Column(db.Text)
    version = db.Column(db.String(64), default='1.0')
    enabled = db.Column(db.Boolean, default=True, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    domains = db.relationship('ComplianceDomain', backref='framework',
                              cascade='all, delete-orphan', lazy=True)
    workbooks = db.relationship('ComplianceWorkbook', backref='framework',
                                lazy=True)


class ComplianceDomain(db.Model):
    __tablename__ = 'compliance_domain'
    id = db.Column(db.Integer, primary_key=True)
    framework_id = db.Column(db.Integer, db.ForeignKey('compliance_framework.id', ondelete='CASCADE'), nullable=False)
    code = db.Column(db.String(64), nullable=False)
    name = db.Column(db.String(200), nullable=False)
    requirements = db.relationship('ComplianceRequirement', backref='domain',
                                   cascade='all, delete-orphan', lazy=True)
    __table_args__ = (db.UniqueConstraint('framework_id', 'code', name='uq_compliance_domain_code'),)


class ComplianceRequirement(db.Model):
    __tablename__ = 'compliance_requirement'
    id = db.Column(db.Integer, primary_key=True)
    domain_id = db.Column(db.Integer, db.ForeignKey('compliance_domain.id', ondelete='CASCADE'), nullable=False)
    code = db.Column(db.String(64), nullable=False)
    title = db.Column(db.String(300), nullable=False)
    description = db.Column(db.Text)
    controls = db.relationship('ComplianceControl', backref='requirement',
                               cascade='all, delete-orphan', lazy=True)
    __table_args__ = (db.UniqueConstraint('domain_id', 'code', name='uq_compliance_requirement_code'),)


class ComplianceControl(db.Model):
    __tablename__ = 'compliance_control'
    id = db.Column(db.Integer, primary_key=True)
    requirement_id = db.Column(db.Integer, db.ForeignKey('compliance_requirement.id', ondelete='CASCADE'), nullable=False)
    code = db.Column(db.String(100), nullable=False)
    title = db.Column(db.String(300), nullable=False)
    description = db.Column(db.Text)
    control_type = db.Column(db.String(30), default='automated', nullable=False)
    status = db.Column(db.String(30), default='not_assessed', nullable=False)
    weight = db.Column(db.Float, default=1.0, nullable=False)
    __table_args__ = (db.UniqueConstraint('requirement_id', 'code', name='uq_compliance_control_code'),)


class ComplianceWorkbook(db.Model):
    __tablename__ = 'compliance_workbook'
    id = db.Column(db.Integer, primary_key=True)
    framework_id = db.Column(db.Integer, db.ForeignKey('compliance_framework.id', ondelete='SET NULL'))
    name = db.Column(db.String(255), nullable=False)
    original_filename = db.Column(db.String(255))
    file_sha256 = db.Column(db.String(64), nullable=False, unique=True)
    imported_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)
    imported_by = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'))
    active = db.Column(db.Boolean, default=True, nullable=False)
    records = db.relationship('ComplianceWorkbookRecord', backref='workbook',
                              cascade='all, delete-orphan', lazy=True)
    importer = db.relationship('User', foreign_keys=[imported_by])


class ComplianceWorkbookRecord(db.Model):
    __tablename__ = 'compliance_workbook_record'
    id = db.Column(db.Integer, primary_key=True)
    workbook_id = db.Column(db.Integer, db.ForeignKey('compliance_workbook.id', ondelete='CASCADE'), nullable=False)
    control_id = db.Column(db.Integer, db.ForeignKey('compliance_control.id', ondelete='SET NULL'))
    sheet_name = db.Column(db.String(150), nullable=False)
    record_type = db.Column(db.String(50), nullable=False)
    record_key = db.Column(db.String(150))
    ...
    __table_args__ = (
        db.Index('ix_compliance_workbook_record_sheet', 'workbook_id', 'sheet_name'),
        db.Index('ix_compliance_workbook_record_key', 'workbook_id', 'record_key'),
    )
...
class ComplianceRule(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(200), nullable=False)
    source = db.Column(db.String(50), default='wazuh', nullable=False)
    condition_json = db.Column(db.Text, nullable=False, default='{}')
    severity = db.Column(db.String(20), default='medium', nullable=False)
    enabled = db.Column(db.Boolean, default=True, nullable=False)
    schedule = db.Column(db.String(20), default='hourly', nullable=False)
    created_by = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'))
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    controls = db.relationship('ComplianceRuleControl', backref='rule',
                               cascade='all, delete-orphan', lazy=True)

    def get_condition(self):
        try:
            return json.loads(self.condition_json or '{}')
        except (TypeError, ValueError):
            return {}

    def set_condition(self, value):
        self.condition_json = json.dumps(value or {})


class ComplianceRuleControl(db.Model):
    __tablename__ = 'compliance_rule_control'
    id = db.Column(db.Integer, primary_key=True)
    rule_id = db.Column(db.Integer, db.ForeignKey('compliance_rule.id', ondelete='CASCADE'), nullable=False)
    control_id = db.Column(db.Integer, db.ForeignKey('compliance_control.id', ondelete='CASCADE'), nullable=False)
    control = db.relationship('ComplianceControl')
    __table_args__ = (db.UniqueConstraint('rule_id', 'control_id', name='uq_compliance_rule_control'),)
...
class ComplianceResult(db.Model):
    ...
    rule_id = db.Column(db.Integer, db.ForeignKey('compliance_rule.id', ondelete='CASCADE'), nullable=False)
    control_id = db.Column(db.Integer, db.ForeignKey('compliance_control.id', ondelete='CASCADE'), nullable=False)
    server_key = db.Column(db.String(32), default='primary', nullable=False)
...
class ComplianceFinding(db.Model):
    ...
    verification_status = db.Column(db.String(30), default='not_requested', nullable=False)
    ...
    rule_id = db.Column(db.Integer, db.ForeignKey('compliance_rule.id', ondelete='SET NULL'))
    control_id = db.Column(db.Integer, db.ForeignKey('compliance_control.id', ondelete='SET NULL'))
    assigned_user_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'))
    ...
    evidence = db.relationship('ComplianceEvidence', backref='finding',
                               cascade='all, delete-orphan', lazy=True)
    risks = db.relationship('ComplianceRisk', backref='finding',
                            cascade='all, delete-orphan', lazy=True)
    remediations = db.relationship('ComplianceRemediation', backref='finding',
                                   cascade='all, delete-orphan', lazy=True)
...
class ComplianceEvidence(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    finding_id = db.Column(db.Integer, db.ForeignKey('compliance_finding.id', ondelete='CASCADE'), nullable=True)
    control_id = db.Column(db.Integer, db.ForeignKey('compliance_control.id', ondelete='SET NULL'), nullable=True)
    source = db.Column(db.String(50), nullable=False)
    server_key = db.Column(db.String(32), default='primary', nullable=False)
    source_index = db.Column(db.String(255))
    ...
    freshness_days = db.Column(db.Integer, default=30)
...
class ComplianceRisk(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    finding_id = db.Column(db.Integer, db.ForeignKey('compliance_finding.id', ondelete='SET NULL'))
    name = db.Column(db.String(300), nullable=False)
    description = db.Column(db.Text)
    likelihood = db.Column(db.Integer, default=3, nullable=False)
    impact = db.Column(db.Integer, default=3, nullable=False)
    inherent_risk = db.Column(db.Integer, default=9, nullable=False)
    residual_risk = db.Column(db.Integer, default=9, nullable=False)
    owner_id = db.Column(db.Integer, db.ForeignKey('user.id', ondelete='SET NULL'))
    treatment = db.Column(db.String(30), default='mitigate', nullable=False)
    due_date = db.Column(db.DateTime)
    status = db.Column(db.String(30), default='open', nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class ComplianceRemediation(db.Model):
    __tablename__ = 'compliance_remediation'
    id = db.Column(db.Integer, primary_key=True)
    finding_id = db.Column(db.Integer, db.ForeignKey('compliance_finding.id', ondelete='CASCADE'), nullable=False)
    title = db.Column(db.String(300), nullable=False)
