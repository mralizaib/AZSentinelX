...
threat_intel_bp.before_request(make_blueprint_permission_check('threat_intel'))
...
def list_items():
    ...
    infra_relevant = request.args.get('infra_relevant', '')
    ...
    if infra_relevant == 'true':
        from sqlalchemy import or_
        # Show only items whose title or description contains keywords relevant to
        # this environment: Windows/Microsoft, Ubuntu/Linux, pfSense/OPNsense, Cisco.
        INFRA_KEYWORDS = [
            'windows', 'microsoft', 'defender', 'active directory', 'outlook',
            'rdp', 'smb', 'ntlm', 'kerberos', 'exchange', 'sharepoint', 'office',
            '.net', 'powershell', 'win32', 'iis',
            'ubuntu', 'linux', 'openssl', 'kernel', 'bash',
            'pfsense', 'opnsense', 'firewall',
            'cisco', 'catalyst', 'ios xe', 'smart switch', 'nexus',
        ]
        conditions = [
            ThreatIntelItem.title.ilike(f'%{kw}%')
            for kw in INFRA_KEYWORDS
        ] + [
            ThreatIntelItem.description.ilike(f'%{kw}%')
            for kw in INFRA_KEYWORDS
        ]
...
def get_item(item_id):
    ...
    if corr:
        correlation_data = {
            ...
            'potential_risk_agents': corr.get_potential_risk_agents(),
        }
...
def download_csv(item_id):
    ...
    try:
        ...
        writer.writerow(['REBIZ Sentinel X — Vulnerability Report'])
...
def download_xlsx(item_id):
    ...
    try:
        ...
        set_cell(ws, 1, 1, 'REBIZ Sentinel X — Vulnerability Report', font=title_font, fill=hdr_fill, align=Alignment(horizontal='center'))
...
def download_pdf(item_id):
    ...
    try:
        ...
        # ── Cover / Header ─────────────────────────────────────────
        hdr_data = [[
            Paragraph('<b>REBIZ Sentinel X</b>', ParagraphStyle('h1', fontName='Helvetica-Bold', fontSize=14, textColor=C_WHITE)),
        ...
        footer_data = [[
            ...
            Paragraph(f'REBIZ Sentinel X  ·  {datetime.utcnow().strftime("%Y-%m-%d")}',
...
@threat_intel_bp.route('/api/threat-intel/items/<int:item_id>/coverage-gap.csv')
@login_required
def export_coverage_gap(item_id):
    """
    Download a CSV of all coverage-gap agents for this threat intel item — i.e.
    agents whose platform matches the CVE target but were NOT detected by the
    Wazuh Vulnerability Detector.  Analysts can use this list to manually
    trigger syscollector / run inventory scripts on those endpoints.
    """
    from models import ThreatIntelItem
    item = ThreatIntelItem.query.get_or_404(item_id)
    corr = item.correlation
    if not corr:
        return jsonify({'error': 'No correlation data found.'}), 400

    potential = corr.get_potential_risk_agents()

    buf = io.StringIO()
    writer = csv.writer(buf)
    writer.writerow(['agent_id', 'agent_name', 'ip_address', 'os', 'platform', 'location'])
    for a in potential:
        writer.writerow([
            a.get('id', ''),
            a.get('name', ''),
    ...
    cve_slug = item.title.split()[0].replace('/', '-') if item.title else f'item{item_id}'
    filename = f"coverage_gap_{cve_slug}_{datetime.utcnow().strftime('%Y%m%d')}.csv"
    return Response(
        buf.getvalue(),
        mimetype='text/csv',
        headers={'Content-Disposition': f'attachment; filename="{filename}"'},
