from flask import Blueprint, render_template, jsonify, request, flash, current_app, redirect, url_for
...
# Suppressed rule IDs — noisy/informational rules excluded from dashboard security metrics
SUPPRESSED_RULE_IDS = [750, 60642, 752, 550, 60106, 4804, 60104, 61102, 4803, 60608]

@dashboard_bp.route('/dashboard')
@login_required
def index():
    # Redirect auditors to their dedicated compliance dashboard
    if current_user.role == 'auditor':
        return redirect(url_for('auditor_dashboard.index'))
    ...
    try:
        ...
        if agents_status and 'data' in agents_status and isinstance(agents_status['data'], dict) and 'affected_items' in agents_status['data']:
            ...
            if isinstance(agents, list):
                # Use total_affected_items from the API response for the true count
                # (len(agents) is capped by the request limit and will under-count)
                agent_stats['total'] = agents_status['data'].get('total_affected_items', len(agents))
                for agent in agents:
...
def threat_analysis():
    try:
        ...
        locations = []
        if response and 'aggregations' in response and 'locations' in response['aggregations']:
...
def alerts_timeline():
    try:
        ...
        body = {
            "size": 0,
            "query": {"bool": {
                "filter": [{"range": {"@timestamp": {"gte": start_time.isoformat(), "lte": end_time.isoformat()}}}],
...
@dashboard_bp.route('/api/dashboard/agents/export')
@login_required
def export_agents():
    """Export agents list as CSV"""
    try:
        import csv
        import io
        ...
        # Header
        writer.writerow(['ID', 'Name', 'IP Address', 'Status', 'Group', 'Location', 'OS', 'Last Keep Alive'])

        for agent in agents_list:
            groups = agent.get('group', [])
            group_str = ', '.join(groups) if isinstance(groups, list) else (groups or 'N/A')
            ...
            writer.writerow([
                ...
                group_str if group_str else 'N/A',
                agent.get('node_name', 'N/A'),
                os_str,
                agent.get('lastKeepAlive', 'N/A'),
            ])
            
        output.seek(0)
        response = make_response(output.getvalue())
        response.headers['Content-Type'] = 'text/csv'
        response.headers['Content-Disposition'] = f'attachment; filename=agents_export_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        return response
    except Exception as e:
