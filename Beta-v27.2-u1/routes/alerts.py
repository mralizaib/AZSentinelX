...
from timezone_utils import (
    format_timestamp_for_server,
    get_server_timezone,
    get_server_timezone_name,
    parse_alert_timestamp,
    timezone_label,
)
from config import Config
from defender_status import normalize_defender_event, pick as pick_defender_field
...
def _result_timestamp(result):
    """Return a sortable timestamp for mixed alert/archive results."""
    source = result.get('source') or {}
    value = (
        result.get('timestamp')
        or source.get('@timestamp')
        or source.get('timestamp')
        or source.get('predecoder', {}).get('timestamp')
    )
    try:
        return parse_alert_timestamp(value)
    except (TypeError, ValueError, OverflowError):
        return datetime.min.replace(tzinfo=_UTC)


def _utc_iso(value):
    """Serialize a timezone-aware datetime in a form OpenSearch accepts."""
    return value.astimezone(_UTC).isoformat()


def _resolve_time_range(args, server_key='primary'):
    """Resolve filters using the selected Wazuh server's local timezone."""
    now = datetime.now(_UTC)
    server_zone = get_server_timezone(server_key)
    server_timezone_name = get_server_timezone_name(server_key)
    ...
    if time_range in relative_ranges:
        start = now - relative_ranges[time_range]
        return _utc_iso(start), _utc_iso(now), {
            'mode': 'relative',
            'start': _utc_iso(start),
            'end': _utc_iso(now),
            'label': f'last {time_range}',
            'timezone': timezone_label(server_key),
    ...
    if time_range in ('calendar', 'date') or start_date_raw:
        if not start_date_raw:
            raise ValueError('A start date is required for calendar filtering')
        try:
            start_date = datetime.strptime(str(start_date_raw).strip(), '%Y-%m-%d').date()
            end_date = datetime.strptime(str(end_date_raw).strip(), '%Y-%m-%d').date()
        except (TypeError, ValueError):
            raise ValueError('Dates must use YYYY-MM-DD format')
        if end_date < start_date:
            raise ValueError('End date cannot be earlier than start date')

        start_local = datetime.combine(start_date, datetime.min.time(), tzinfo=server_zone)
        end_local = datetime.combine(
            end_date + timedelta(days=1), datetime.min.time(), tzinfo=server_zone
        )
        start = _utc_iso(start_local)
        end = _utc_iso(end_local - timedelta(microseconds=1))
        label = (
            start_date_raw if start_date == end_date
            else f'{start_date_raw} to {end_date_raw}'
        )
        return start, end, {
            'mode': 'calendar',
    ...
    # Preserve compatibility with existing custom ISO query parameters.
    start = args.get('start_time')
    end = args.get('end_time') or _utc_iso(now)
    if not start:
        raise ValueError('A start time is required for a custom range')
    return start, end, {
        'mode': 'custom',
        'start': start,
        'end': end,
...
def _merge_search_sources(source_results, offset, limit, sort_field, sort_order):
    """Merge independent alert/archive pages into one global result page."""
    errors = [
        result.get('error')
    ...
    if not usable_results:
        return {'error': '; '.join(errors) or 'No search source available'}

    merged = []
    seen = set()
    for result in usable_results:
        for item in result.get('results', []):
            identity = (item.get('index'), item.get('id'))
            if identity in seen:
                continue
            seen.add(identity)
            merged.append(item)
    ...
    if sort_field in ('@timestamp', 'timestamp'):
        merged.sort(key=_result_timestamp, reverse=reverse)
    elif sort_field == '_score':
        merged.sort(key=lambda item: item.get('score') or 0, reverse=reverse)

    combined = {
        'total': sum(result.get('total', 0) or 0 for result in usable_results),
        'results': merged[offset:offset + limit],
    }
    if errors:
        combined['warning'] = (
            'Some records could not be searched: ' + '; '.join(errors)
        )
    return combined
...
def index():
    ...
    active_server_key = session.get('wazuh_server', 'primary')
    active_server = Config.get_server(active_server_key)
    return render_template(
        'alerts.html',
        alerts=alerts,
        rule_id=rule_id,
        rule_desc=rule_desc,
        active_server_timezone=active_server.get('timezone', 'UTC'),
        active_server_timezone_label=timezone_label(active_server_key),
    )
...
def get_alerts():
    ...
    try:
        ...
        try:
            start_time, end_time, date_range = _resolve_time_range(
                request.args, opensearch.server_key
            )
        except ValueError as range_error:
            return jsonify({'error': str(range_error)}), 400
        ...
        no_explicit_severity_filter = not severity_levels and not single_severity
        global_search = (
            no_explicit_severity_filter
            and bool((search_query or '').strip())
        )

        if not severity_levels:
            severity_levels = ['critical', 'high']  # Default levels

        syslog_requested = 'syslog' in severity_levels
        alert_severity_levels = [
            level for level in severity_levels if level != 'syslog'
        ]

        decoder_query = (
            request.args.get('decoder')
            or request.args.get('decoder_query')
            or ''
        ).strip()
        rule_id = request.args.get('rule_id')
        if rule_id:
        ...
        # Keep the two data sources isolated. A text search must not silently
        # add archive/Syslog records to the alert result set, and selecting
        # Syslog must not mix regular Wazuh alerts into the Syslog view.
        syslog_only = syslog_requested
        if syslog_only:
            # Syslog is a source/view selector, not a decoder name. With no
            # decoder entered, search both Wazuh alert and archive indices so
            # records from OpenVPN, Windows, firewalls, and every other
            # decoder are discoverable. When a decoder is entered, the same
            # exact filter is applied to both searches.
            #
            # Fetch one page from each source, then merge and sort before
            # applying the requested offset. This preserves a correct
            # cross-index page without mixing the normal alert view.
            fetch_limit = min(max(limit + offset, limit), 10000)
            alert_results = opensearch.search_alerts(
                severity_levels=[],
                ...
                decoder_query=decoder_query or None,
                normalize_syslog=True,
            )
            archive_results = opensearch.search_syslog_events(
                start_time=start_time,
                end_time=end_time,
            ...
            results = _merge_search_sources(
                (alert_results, archive_results),
                offset,
                limit,
                sort_field,
                sort_order,
            ...
            # A search with no selected category is intentionally global:
            # remove the normal critical/high default and search the regular
            # alert index plus all archive decoders. Archive hits are
            # normalized as Syslog records; regular alert hits retain their
            # native alert structure so the result table can render both.
            fetch_limit = min(max(limit + offset, limit), 10000)
            alert_results = opensearch.search_alerts(
                severity_levels=[],
                start_time=start_time,
            ...
            # An explicitly selected severity/category remains restrictive,
            # even when a search term is supplied.
            requested_alert_levels = alert_severity_levels
            results = opensearch.search_alerts(
                severity_levels=requested_alert_levels,
                start_time=start_time,
                end_time=end_time,
                limit=limit,
                offset=offset,
        ...
        if results.get('error'):
            return jsonify({
                'error': results['error'],
                'server_key': opensearch.server_key,
                'server_name': opensearch.server_name,
                'server_timezone': opensearch.server_timezone,
                'server_timezone_label': timezone_label(opensearch.server_key),
                'date_range': date_range,
            }), 502

        results['date_range'] = date_range
        results['server_key'] = opensearch.server_key
        results['server_name'] = opensearch.server_name
        results['server_timezone'] = opensearch.server_timezone
        results['server_timezone_label'] = timezone_label(opensearch.server_key)
        results['search_scope'] = (
            'syslog' if syslog_only else ('global' if global_search else 'alerts')
        )
...
def view_alert(alert_id):
    ...
    try:
        ...
        if timestamp:
            alert_data['timestamp_raw'] = timestamp
            alert_data['timestamp_display'] = format_timestamp_for_server(
                timestamp, opensearch.server_key
            )
        else:
            alert_data['timestamp_raw'] = ''
            alert_data['timestamp_display'] = ''
        ...
        if not isinstance(rule, dict):
            rule = {}
            source['rule'] = rule
        # Keep the template safe for archive records and partially decoded
        # events that do not carry a Wazuh rule object.
        rule.setdefault('description', '')
        rule.setdefault('level', 0)
        rule.setdefault('id', '')
        rule.setdefault('groups', [])
        alert_data['rule'] = rule
        ...
        if any(kw in desc or kw in groups_str for kw in defender_keywords):
            ...
            defender_status = normalize_defender_event(eventdata, rule.get('description', ''))

            threat_name  = defender_status['threat_name']
            action       = defender_status['action_name']
            file_path    = pick_defender_field(
                eventdata, 'path', 'Path', 'process', 'Process',
                'processName', 'filePath', 'file',
            )
            sev_name     = pick_defender_field(
                eventdata, 'severity name', 'severityName', 'Severity Name',
                'severity', 'Severity',
            )
            det_user     = pick_defender_field(
                eventdata, 'detection user', 'detectionUser', 'Detection User',
                'user', 'User', 'processUser',
            )
            product_name = pick_defender_field(
                eventdata, 'product name', 'productName', 'Product Name',
                'product', 'Product',
            )
            detection_source = pick_defender_field(
                eventdata, 'source name', 'sourceName', 'Source Name',
                'detection source', 'detectionSource', 'provider',
            )

            action_time = pick_defender_field(
                eventdata, 'actionTime', 'action time', 'Action Time',
                'remediationTime', 'Remediation Time',
                'detectionTime', 'timeCreated',
            )
            executed_by = pick_defender_field(
                eventdata, 'executedBy', 'Executed By', 'provider',
            ...
            reason = pick_defender_field(
                eventdata, 'reason', 'Reason', 'noActionReason',
                'No Action Reason', 'errorDescription', 'errorCode', 'error',
            )
            category_name = pick_defender_field(
            ...
            defender_info = {
                ...
                'action':           action,
                'action_taken':     defender_status['action_taken'],
                'action_status':    defender_status['action_status'],
                'action_status_label': defender_status['action_status_label'],
                'action_type':      defender_status['action_type'],
                'additional_action_required': defender_status['additional_action_required'],
                'execution_status': defender_status['execution_status'],
                'action_time':      action_time,
                'executed_by':      executed_by,
                'execution_result': defender_status['execution_result'],
                'reason':           reason if defender_status['additional_action_required'] else '',
                'error_code':       defender_status['error_code'],
                'error_description': defender_status['error_description'],
                'additional_actions': defender_status['additional_actions'],
                'detection_source': detection_source,
                'auto_remediate':    bool(source.get('auto_remediate') or source.get('autoRemediate')),
                'notification_action_required': defender_status['notification_action_required'],
                'notification_message': defender_status['notification_message'],
                'file_path':        file_path,
                ...
                'status':           'Resolved' if defender_status['status_resolved'] else 'Active',
                'status_resolved':  defender_status['status_resolved'],
                'raw_eventdata':    eventdata,
            }
            # Expose the canonical interpretation to any downstream renderer
            # without modifying the original Defender payload.
            alert_data['defender_remediation'] = defender_status
...
def export_alerts():
    ...
    try:
        ...
        try:
            start_time, end_time, _date_range = _resolve_time_range(
                request.args, opensearch.server_key
            )
        except ValueError as range_error:
            return jsonify({'error': str(range_error)}), 400
        ...
        if export_format == 'csv':
            return export_alerts_csv(alerts_data, opensearch.server_key)
        elif export_format == 'xlsx':
            return export_alerts_xlsx(alerts_data, opensearch.server_key)
        elif export_format == 'pdf':
            from report_generator import ReportGenerator
            generator = ReportGenerator(server_key=opensearch.server_key)
...
def export_alerts_csv(alerts_data, server_key='primary'):
    ...
    # Write header
    writer.writerow([
        f'Timestamp ({timezone_label(server_key)})',
        'User',
        'Agent Name',
        'Agent ID', 
        'Agent IP',
    ...
    for alert in alerts_data:
        ...
        try:
            if timestamp != 'N/A':
                formatted_time = format_timestamp_for_server(
                    timestamp, server_key, include_zone=True
                )
            else:
                formatted_time = 'N/A'
        except:
...
def export_alerts_xlsx(alerts_data, server_key='primary'):
    ...
    try:
        ...
        # Write headers
        headers = [
            f'Timestamp ({timezone_label(server_key)})', 'User', 'Agent Name', 'Agent ID', 'Agent IP',
...
def export_alerts_pdf(alerts_data, server_key='primary'):
    ...
    try:
        ...
        export_info = Paragraph(
            f"Generated on: {format_timestamp_for_server(datetime.now(_UTC).isoformat(), server_key)}",
            styles['Normal']
        )
        ...
        for alert in alerts_data[:100]:  # Limit to first 100 for PDF
            ...
            try:
                if timestamp != 'N/A':
                    formatted_time = format_timestamp_for_server(
                        timestamp, server_key, include_zone=True, include_seconds=False
                    )
                else:
                    formatted_time = 'N/A'
            except:
