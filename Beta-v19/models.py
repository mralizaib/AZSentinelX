...
class XDRIncident(db.Model):
    """A correlated incident that groups multiple ITDR detections."""
    id               = db.Column(db.Integer, primary_key=True)
    ...
    categories       = db.Column(db.Text)       # JSON list of detection categories
    affected_agents  = db.Column(db.Text)       # JSON list of agent names
    source_ips       = db.Column(db.Text)       # JSON list of source IPs
    detection_count  = db.Column(db.Integer, default=0)
    created_at       = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at       = db.Column(db.DateTime, default=datetime.utcnow)
    resolved_at      = db.Column(db.DateTime, nullable=True)
    ai_summary       = db.Column(db.Text)
    recommended_actions = db.Column(db.Text)    # JSON list of action strings

    detections = db.relationship(
        'ITDRDetection', backref='incident', lazy='dynamic',
        foreign_keys='ITDRDetection.incident_id',
    )
    ...
    def get_affected_agents(self):
        return json.loads(self.affected_agents) if self.affected_agents else []

    def get_source_ips(self):
        return json.loads(self.source_ips) if self.source_ips else []

    def get_recommended_actions(self):
        return json.loads(self.recommended_actions) if self.recommended_actions else []

    def __repr__(self):
        return f'<XDRIncident {self.incident_number} [{self.status}]>'


class ITDRDetection(db.Model):
    """A single detected identity/endpoint threat event."""
    id            = db.Column(db.Integer, primary_key=True)
    detection_id  = db.Column(db.String(64), unique=True, nullable=False)
    rule_id       = db.Column(db.String(20))        # e.g. ITDR-001
    rule_name     = db.Column(db.String(200))
    category      = db.Column(db.String(50))        # credential_attack | privilege_escalation | …
    severity      = db.Column(db.String(20))        # critical | high | medium | low
    source_ip     = db.Column(db.String(60))
    target_agents = db.Column(db.Text)              # JSON list of agent names
    event_count   = db.Column(db.Integer, default=1)
    first_seen    = db.Column(db.DateTime)
    last_seen     = db.Column(db.DateTime, default=datetime.utcnow)
    detected_at   = db.Column(db.DateTime, default=datetime.utcnow)
    details       = db.Column(db.Text)              # JSON: descriptions, rule_ids, etc.
    raw_rule_ids  = db.Column(db.Text)              # JSON list of Wazuh rule IDs
    incident_id   = db.Column(db.Integer, db.ForeignKey('xdr_incident.id'), nullable=True)
    alert_sent    = db.Column(db.Boolean, default=False)
    remediated    = db.Column(db.Boolean, default=False)

    def get_target_agents(self):
        return json.loads(self.target_agents) if self.target_agents else []
    ...
    def __repr__(self):
        return f'<ITDRDetection {self.rule_id} [{self.severity}]>'


class RemediationPolicy(db.Model):
    """User-defined automated response policy."""
    id                      = db.Column(db.Integer, primary_key=True)
    name                    = db.Column(db.String(200), nullable=False)
    description             = db.Column(db.Text)
    enabled                 = db.Column(db.Boolean, default=True)
    trigger_categories      = db.Column(db.Text)    # JSON list; empty = any
    trigger_severities      = db.Column(db.Text)    # JSON list; empty = any
    trigger_min_event_count = db.Column(db.Integer, default=1)
    action_type             = db.Column(db.String(50))   # email | webhook | escalate
    action_email            = db.Column(db.String(500))
    action_webhook_url      = db.Column(db.String(500))
    action_webhook_secret   = db.Column(db.String(200))
    created_at              = db.Column(db.DateTime, default=datetime.utcnow)
    created_by              = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)

    def get_trigger_categories(self):
        return json.loads(self.trigger_categories) if self.trigger_categories else []

    def get_trigger_severities(self):
        return json.loads(self.trigger_severities) if self.trigger_severities else []

    def __repr__(self):
        return f'<RemediationPolicy {self.name}>'


class NoiseFilter(db.Model):
    """Configurable log noise suppression rule."""
    id               = db.Column(db.Integer, primary_key=True)
    name             = db.Column(db.String(200), nullable=False)
    filter_type      = db.Column(db.String(30))   # rule_id | rule_group | min_level | description_contains | agent
    filter_value     = db.Column(db.String(500))  # comma-separated rule IDs, group name, level int, etc.
    action           = db.Column(db.String(20), default='suppress')  # suppress | deprioritize
    enabled          = db.Column(db.Boolean, default=True)
    is_system        = db.Column(db.Boolean, default=False)   # pre-seeded defaults
    estimated_daily  = db.Column(db.Integer, default=0)       # estimated daily suppression volume
    notes            = db.Column(db.Text)
    created_at       = db.Column(db.DateTime, default=datetime.utcnow)
    created_by       = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=True)

    def __repr__(self):
        return f'<NoiseFilter {self.name} [{self.filter_type}={self.filter_value}]>'


class ActiveThreatNotification(db.Model):
    """
    Tracks every high/critical alert that triggered an 'Active Threat Detected'
    email notification.  Used for deduplication so each unique OpenSearch alert
    ...
    rule_level           = db.Column(db.Integer)
    rule_description     = db.Column(db.String(500))
    ...
    recipients_notified  = db.Column(db.Text)                 # comma-separated emails
    raw_alert_json       = db.Column(db.Text)                 # truncated _source JSON

    def __repr__(self):
        return f'<ActiveThreatNotification rule={self.rule_id} agent={self.agent_name} sev={self.severity}>'


class RemediationAction(db.Model):
    """Log of automated remediation actions taken."""
    id             = db.Column(db.Integer, primary_key=True)
    policy_id      = db.Column(db.Integer, db.ForeignKey('remediation_policy.id'), nullable=True)
    detection_id   = db.Column(db.Integer, db.ForeignKey('itdr_detection.id'), nullable=True)
    incident_id    = db.Column(db.Integer, db.ForeignKey('xdr_incident.id'), nullable=True)
    ...
    policy = db.relationship('RemediationPolicy', backref='actions')
