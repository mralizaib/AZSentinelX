...
def fetch_wazuh_cti(limit=500) -> list:
    ...
    try:
        ...
        # Wazuh status values that mean "still vulnerable / unpatched"
        _ACTIVE_STATUSES = {'active', 'still applicable', 'pending confirmation', 'unpatched'}
        # Anything else (solved, fixed, patched, obsolete, …) is treated as Solved

        def _classify_agents(agg_bucket: dict):
            """
            Iterate the agents_with_status bucket and return (active_list, solved_list).
            Classification is case-insensitive so it works regardless of Wazuh version.
            """
            active_list, solved_list = [], []
            for ab in agg_bucket.get('agents_with_status', {}).get('buckets', []):
                ...
                entry = {'name': agent_name, 'ip': '', 'location': ''}
                if raw_status.lower().strip() in _ACTIVE_STATUSES:
                    active_list.append(entry)
                else:
                    # No status or any "solved" variant → treat as patched
                    solved_list.append(entry)
            return active_list, solved_list
        ...
        for bucket in buckets:
            ...
            active_agents, solved_agents = _classify_agents(bucket)
