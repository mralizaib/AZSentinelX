...
def _patch_xml_fields(xml_str, patches):
    ...
    # Accept both the documented iterable-of-tuples form and a mapping.  The
    # device-template code passes a mapping because it is easier to read; the
    # old implementation iterated mapping keys and could raise ValueError,
    # silently disabling the template-clone path.
...
class DVRClient:
    def __init__(self, ip, port, username, password, use_https=False):
        ...
        # Retain the credentials for ISAPI payloads that require the current
        # administrator password in <loginPassword>.  HTTP Digest still
        # authenticates every request separately through self.auth.
...
<HolidayList>
  <Holiday>
    ...
    # This is the namespace used by the supplied working PowerShell tool for
    # Hikvision V4.83 firmware.  Some OEM devices use their own namespace, so
    # it is tried first only as a compatibility variant.
    _ISAPI_USER_NS = "http://www.isapi.org/ver20/XMLSchema"

    def _build_reference_user_xml(self, username, password, user_level,
                                  namespace=None):
        """Build the user payload used by the supplied PowerShell reference.

        Newer Hikvision/LTS firmware validates ``loginPassword`` and the
        version/namespace attributes even though older firmware accepted a
        smaller User document.
        """
        ns = namespace or self._ISAPI_USER_NS
        return f"""<?xml version="1.0" encoding="UTF-8"?>
<User version="2.0" xmlns="{xml_escape(ns)}">
    <id>0</id>
    <userName>{xml_escape(str(username))}</userName>
    <loginPassword>{xml_escape(str(self.password))}</loginPassword>
    <password>{xml_escape(str(password))}</password>
    <bondIpList>
        <bondIp>
            <id>1</id>
    ...
    <userLevel>{xml_escape(str(user_level))}</userLevel>
    <attribute>
    ...
    def _build_create_user_xml_from_template(self, username, password, user_level):
        ...
        try:
            ...
            # A GET response may not contain loginPassword because the device
            # never returns it.  Insert it after userName when absent.
            if not _re.search(r'<(?:[^:>\s/]+:)?loginPassword(?:\s[^>]*)?>',
                              user_block):
                user_block = _re.sub(
                    r'(</(?:[^:>\s/]+:)?userName>)',
                    r'\n  <loginPassword>' + xml_escape(str(self.password)) +
                    r'</loginPassword>\1',
                    user_block, count=1
                )
    ...
    def create_user(self, username, password, role="viewer"):
        ...
        post_exc = None
        # The supplied PowerShell script's exact payload is the primary
        # strategy.  Then try the device-detected namespace, a cloned device
        # template, and finally the legacy minimal document for older units.
        variants = [
            self._build_reference_user_xml(username, password, user_level),
        ]
        detected_ns = getattr(self, "_cached_device_ns", None)
        if detected_ns and detected_ns != self._ISAPI_USER_NS:
            variants.append(self._build_reference_user_xml(
                username, password, user_level, namespace=detected_ns))
        cloned = self._build_create_user_xml_from_template(
            username, password, user_level)
        if cloned:
            variants.append(cloned)
        variants.append(f"""<?xml version="1.0" encoding="UTF-8"?>
<User>
  <id>0</id>
  <userName>{xml_escape(str(username))}</userName>
  <password>{xml_escape(str(password))}</password>
  <bondIpList>
    <bondIp><id>1</id><ipAddress>0.0.0.0</ipAddress><ipv6Address>::</ipv6Address></bondIp>
  </bondIpList>
  <macAddress></macAddress>
  <userLevel>{xml_escape(str(user_level))}</userLevel>
  <attribute><inherent>false</inherent></attribute>
        ...
        for xml in variants:
            ...
            try:
                ...
                post_exc = e
                logger.warning(
                    f"POST /Security/users variant failed for {username}: {e}")

        logger.warning(f"All POST /Security/users variants failed for {username}; "
                       "falling back to GET-then-PUT full UserList approach")
    ...
    def _update_user_password(self, uid, username, password, user_level):
        ...
        variants = [
            self._build_reference_user_xml(username, password, user_level)
                .replace("<id>0</id>", f"<id>{xml_escape(str(uid))}</id>", 1),
            f"""<?xml version="1.0" encoding="UTF-8"?>
<User>
  <id>{xml_escape(str(uid))}</id>
  <userName>{xml_escape(str(username))}</userName>
  <password>{xml_escape(str(password))}</password>
  <userLevel>{xml_escape(str(user_level))}</userLevel>
</User>""",
        ...
        for xml in variants:
            ...
            try:
                ...
                last_error = e
                logger.warning(f"Update user {uid} variant failed: {e}")
        if last_error:
            raise last_error
    ...
    "cms": {
        ...
        "remote": {
            ...
            "palyBack": True,                # Legacy firmware spelling
            "record": False,
            ...
            "voiceTalk": False,
            "ptzControl": False,
            "transParentChannel": False,
            "contorlLocalOut": False,
            "alarmOutOrUpload": False,
            "backup": False,
        },
        "local": {
            "preview": True,                 # Live View
    ...
    def set_user_permissions(self, uid, username, permission_set):
        ...
        Newer firmware expects the local and remote permission documents to be
        read and written independently.  The supplied working PowerShell
        utility preserves each document returned by the device, changes only
        supported flags (including per-channel flags), and PUTs them to:

          /Security/UserPermission/{id}/localPermission
          /Security/UserPermission/{id}/remotePermission

        A combined UserPermission PUT and the older legacy path remain as
        fallbacks for older firmware.
        """
        spec = self._PERMISSION_SPECS[self._spec_key_for(permission_set)]
        user_type = spec["user_type"]

        def _set_direct_value(parent, tag, value):
            node = _child_by_local_name(parent, tag)
            if node is None:
                uri = (parent.tag.split('}', 1)[0][1:]
                       if parent is not None and '}' in parent.tag else
                       self._detect_device_ns())
                node = ET.SubElement(parent, f"{{{uri}}}{tag}" if uri else tag)
            node.text = "true" if value else "false"

        def _patch_permission_document(raw_xml, fields, channel_fields):
            root = _parse_xml(raw_xml)
            if root is None:
                raise ValueError("Device returned invalid permission XML")

            # Match the PowerShell Set-PermissionValue behavior: update or
            # append direct permission nodes while preserving the root's
            # version, namespace, ordering, and any OEM-specific fields.
            for tag, value in fields.items():
                _set_direct_value(root, tag, value)
            ...
            if channel_list is not None:
                for channel in _children_by_local_name(
                    ...
                    for tag, value in channel_fields.items():
                        _set_direct_value(channel, tag, value)
            return _serialize(root)

        local_path = f"/Security/UserPermission/{uid}/localPermission"
        remote_path = f"/Security/UserPermission/{uid}/remotePermission"

        try:
            local_response = self._get(local_path)
            remote_response = self._get(remote_path)
            local_xml = _patch_permission_document(
                local_response.text, spec["local"], spec.get("channel", {}))
            remote_xml = _patch_permission_document(
                remote_response.text, spec["remote"], spec.get("channel", {}))
            self._put(local_path, local_xml)
            self._put(remote_path, remote_xml)
            logger.info("Set permissions for %s using separate local/remote "
                        "permission endpoints", username)
            return True
        except Exception as separate_error:
            logger.warning(
                "Separate permission workflow for %s failed: %s; trying "
                "combined/legacy permission payload", username, separate_error)

        # Compatibility fallback for older firmware that exposes only the
        # combined document or the legacy users/{id}/userPermission endpoint.
...
<UserPermission version="2.0" xmlns="{xml_escape(ns)}">
  <id>{xml_escape(str(uid))}</id>
  <userID>{xml_escape(str(uid))}</userID>
  <userType>{xml_escape(str(user_type))}</userType>
  <remotePermission>{_fields_xml(spec["remote"])}
  </remotePermission>
  <localPermission>{_fields_xml(spec["local"])}
  </localPermission>
</UserPermission>"""
        try:
    ...
    def get_user_permissions(self, uid):
        ...
        try:
            ...
            for section_name in ("remotePermission", "localPermission"):
                r = self._get(
                    f"/Security/UserPermission/{uid}/{section_name}")
                root = _parse_xml(r.text)
                if root is None:
                    return None
                channels = []
                ...
                if channel_list is not None:
                    for channel in _children_by_local_name(
                        ...
                        for child in list(channel):
                            ...
                            val = (child.text or "").strip().lower()
                            if val in ("true", "false", "1", "0", "yes", "no"):
                                channel_values[tag] = val in (
                                    "true", "1", "yes")
                        channels.append(channel_values)
                perms[section_name]["_channels"] = channels
            return perms
        except Exception as e:
            logger.info(f"Separate permission read for uid {uid} failed: {e}")
            # Older firmware fallback: read the combined document.
            try:
                ...
                for section_name in ("remotePermission", "localPermission"):
                    ...
                    if section_node is not None:
                        ...
                        for child in list(section_node):
                            ...
                            val = (child.text or "").strip().lower()
                            if val in ("true", "false", "1", "0", "yes", "no"):
                                perms[section_name][tag] = val in (
                                    "true", "1", "yes")
                        channel_list = _find_first(
                            section_node, "videoChannelPermissionList")
                        if channel_list is not None:
                            for channel in _children_by_local_name(
                                    channel_list, "videoChannelPermission"):
                                channel_values = {}
                ...
                logger.info(f"Get permissions for uid {uid}: {fallback_error}")
                return None
        ...
        for section_name, fields in (("remotePermission", spec["remote"]), ("localPermission", spec["local"])):
            ...
            for tag, expected_val in fields.items():
                if current_section.get(tag) != expected_val:
                    return False
            expected_channel = spec.get("channel", {})
            for channel in current_section.get("_channels", []):
                for tag, expected_val in expected_channel.items():
                    if channel.get(tag) != expected_val:
                        return False
        return True
