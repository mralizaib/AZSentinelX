...
reports_bp.before_request(make_blueprint_permission_check('reports'))
...
def _make_email_body(report_config, generated_at, downloaded_by, period_start, period_end,
    ...
    logo_html = ''
    if logo_data_uri:
        logo_html = f'<img src="{logo_data_uri}" width="56" height="56" style="object-fit:contain;background:#fff;border-radius:8px;padding:4px;" alt="REBIZ logo">'
    else:
        logo_html = '<div style="width:56px;height:56px;background:rgba(255,255,255,0.2);border-radius:8px;display:inline-flex;align-items:center;justify-content:center;font-weight:700;font-size:16px;color:#fff;">RB</div>'
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          ...
          <td style="padding-left:16px;vertical-align:middle;">
            <div style="font-size:10px;text-transform:uppercase;letter-spacing:2px;color:rgba(255,255,255,0.65);margin-bottom:4px;">REBIZ Sentinel X · v27.2</div>
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        <tr>
          <td style="color:rgba(255,255,255,0.7);font-size:11px;line-height:1.8;">
            <strong style="color:#fff;">REBIZ Sentinel X</strong> · v27.2<br>
...
def generate_report_api():
    ...
    try:
        ...
        if report_config['format'].lower() == 'html':
            ...
            response.headers['Content-Disposition'] = f'attachment; filename=rebiz_sentinel_x_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.pdf'
            return response
    except Exception as e:
...
def send_report_email(report_id):
    ...
    try:
        ...
        subject = f"REBIZ Sentinel X — Security Report: {report_config.name} [{now_pkt}]"
        ...
        if report_config.format.lower() == 'pdf':
            attachments = [{
                'content': report,
                'filename': f"rebiz_sentinel_x_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
