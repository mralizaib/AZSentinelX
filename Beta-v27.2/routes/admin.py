    ...
    try:
        ...
        for alert_config in alert_configs:
            try:
                ...
                # ── FIM configs: use the dedicated FIM sender ─────────────
                if alert_config.is_fim():
                    logger.info(f"FIM config {alert_config.id} — routing to send_fim_alert()")
                    result = email_alerts.send_fim_alert(alert_config)
                    success = result[0] if isinstance(result, tuple) else bool(result)
                    result_msg = result[1] if isinstance(result, tuple) else ''
                    if success:
                        logger.info(f"Successfully sent FIM test alert for config ID {alert_config.id}")
                        success_count += 1
                        flash(f'Successfully sent FIM test alert for "{alert_config.name}" to {recipient}.', 'success')
                    else:
                        logger.error(f"Failed FIM test alert for config ID {alert_config.id}: {result_msg}")
                        error_count += 1
                        flash(f'Failed FIM test alert for "{alert_config.name}": {result_msg}', 'danger')
                    continue

                # ── Standard severity configs ─────────────────────────────
                alerts_data = None
                try:
                    ...
                    alerts_data = {
                        'results': [], 'total': 0,
                        'manual_test': True, 'connection_error': True,
                        'error_message': str(e)
                    }
                ...
                if not alerts_data or not alerts_data.get('results', []):
                    ...
                    alerts_data = {'results': [], 'total': 0, 'manual_test': True}

                logger.info(f"Manually sending alert for config ID {alert_config.id} to {recipient}")
                success, result_msg = email_alerts.send_severity_alert(alert_config, alerts_data)

                if success:
                    logger.info(f"Successfully sent test alert for config ID {alert_config.id}")
                    success_count += 1
                    flash(f'Successfully sent test alert for "{alert_config.name}" to {recipient}.', 'success')
                else:
                    logger.error(f"Failed to send test alert for config ID {alert_config.id}: {result_msg}")
                    error_count += 1
                    flash(f'Failed to send test alert for "{alert_config.name}": {result_msg}', 'danger')
...
@admin_bp.route('/console-logs')
@login_required
def console_logs():
    """Server console log viewer page."""
    return render_template('console_logs.html')


@admin_bp.route('/api/console-logs')
@login_required
def api_console_logs():
    """
    Return buffered server log lines as JSON.
    Query params:
      since  – float Unix timestamp; only lines after this are returned
      limit  – max lines to return (default 200, max 400)
      level  – optional filter: DEBUG / INFO / WARNING / ERROR / CRITICAL
    """
    try:
        from app import _mem_handler
        since = float(request.args.get('since', 0))
        limit = min(int(request.args.get('limit', 200)), 400)
        level_filter = request.args.get('level', '').upper()

        lines = _mem_handler.get_lines(since_ts=since, limit=limit)
        ...
        last_ts = lines[-1]['ts'] if lines else since
        return jsonify({'lines': lines, 'last_ts': last_ts})
    except Exception as e:
        logger.error(f"Console log API error: {e}")
        return jsonify({'lines': [], 'last_ts': 0, 'error': str(e)})