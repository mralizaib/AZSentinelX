...
storage_bp.before_request(make_blueprint_permission_check('storage'))
...
def _serialize_backup(b):
    return {
        ...
        'destination_type': b.destination_type or 'local',
...
def create_backup():
    ...
    Body: { name, log_type, selected_months, destination_type ('local'|'smb'),
            save_path (local only, optional), notes (optional) }
    """
    if not current_user.is_admin():
        return jsonify({'error': 'Unauthorized'}), 403
    try:
        from models import LogBackup, SMBConfig, db
        data             = request.json or {}
        ...
        # Validate SMB config if destination is SMB
        smb_cfg = None
        if destination_type == 'smb':
            smb_cfg = SMBConfig.get_instance()
            if not smb_cfg.server_path:
                return jsonify({'error': 'SMB network share is not configured. Please configure it first.'}), 400
            if not smb_cfg.is_verified:
                return jsonify({'error': 'SMB share not verified. Test the connection before running a backup.'}), 400

        # Resolve local save directory (used for local backups and as a temp staging area for SMB)
        save_dir = save_path if (save_path and destination_type == 'local') else _DEFAULT_BACKUP_DIR
        try:
        ...
        # Build local temp/permanent file path
        safe_name = "".join(c if c.isalnum() or c in '-_.' else '_' for c in name)
        ts_str    = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
        filename  = f"{safe_name}_{ts_str}.ndjson.gz"
        local_file_path = os.path.join(save_dir, filename)

        # Create DB record (file_path updated in thread once we know the final location)
        backup = LogBackup(
            name=name,
            log_type=log_type,
        ...
        # Snapshot SMB config values for the thread (avoid session issues)
        smb_server_path = smb_cfg.server_path if smb_cfg else None
        smb_username    = smb_cfg.username    if smb_cfg else None
        smb_password    = smb_cfg.password    if smb_cfg else None
        smb_domain      = smb_cfg.domain      if smb_cfg else ''
        ...
        def _run_backup():
            with _app.app_context():
                ...
                try:
                    ...
                    # Phase 1: export to local file
                    doc_count = api.export_indices_to_ndjson(
                        indices_to_export, local_file_path, progress_cb=_progress
                    )
                    local_size = os.path.getsize(local_file_path) if os.path.exists(local_file_path) else 0

                    if destination_type == 'smb':
                        # Phase 2: upload to SMB share, then delete local temp
                        from smb_helper import upload_to_smb
                        remote_unc = upload_to_smb(
                            local_file_path,
                            smb_server_path,
                            smb_username,
                            smb_password,
                        ...
                        try:
                            ...
                            pass
                        rec.file_path = remote_unc
                    else:
                        rec.file_path = local_file_path

                    rec.status          = 'complete'
                    rec.doc_count       = doc_count
                    rec.file_size_bytes = local_size
                    rec.completed_at    = datetime.utcnow()
                    logger.info(f"Backup {backup_id} '{name}' complete — {doc_count} docs, dest={destination_type}")
                except Exception as err:
                    rec.status        = 'failed'
                    rec.error_message = str(err)
                    logger.error(f"Backup {backup_id} failed: {err}", exc_info=True)
        ...
        dest_label = 'SMB network share' if destination_type == 'smb' else 'local disk'
        return jsonify({'success': True, 'backup_id': backup_id,
                        'message': f'Backup started — {len(indices_to_export)} indices → {dest_label}.'})
    except Exception as e:
...
@storage_bp.route('/api/storage/smb/config', methods=['GET'])
@login_required
def smb_get_config():
    if not current_user.is_admin():
        return jsonify({'error': 'Unauthorized'}), 403
    try:
        ...
        return jsonify(SMBConfig.get_instance().to_safe_dict())
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@storage_bp.route('/api/storage/smb/config', methods=['POST'])
@login_required
def smb_save_config():
    if not current_user.is_admin():
        return jsonify({'error': 'Unauthorized'}), 403
    try:
        from models import SMBConfig, db
        data = request.json or {}
        cfg = SMBConfig.get_instance()
        cfg.server_path = data.get('server_path', cfg.server_path or '')
        cfg.username    = data.get('username', cfg.username or '')
        cfg.domain      = data.get('domain', cfg.domain or '')
        if data.get('password'):
            cfg.password = data['password']
        cfg.is_verified = False   # reset on change
        cfg.updated_at  = datetime.utcnow()
        db.session.commit()
        return jsonify({'success': True, 'config': cfg.to_safe_dict()})
    except Exception as e:
        return jsonify({'error': str(e)}), 500


@storage_bp.route('/api/storage/smb/test', methods=['POST'])
@login_required
def smb_test_connection():
    if not current_user.is_admin():
        return jsonify({'error': 'Unauthorized'}), 403
    try:
        from models import SMBConfig, db
        from smb_helper import test_smb_connection
        data = request.json or {}
        ...
        # If password not supplied by client, fall back to saved value
        if not password:
            cfg = SMBConfig.get_instance()
            password = cfg.password or ''

        success, message = test_smb_connection(server_path, username, password, domain)

        # Persist result
        cfg = SMBConfig.get_instance()
        cfg.server_path = server_path or cfg.server_path
        cfg.username    = username    or cfg.username
        cfg.domain      = domain      or cfg.domain
        if data.get('password'):
            cfg.password = data['password']
        cfg.is_verified = success
        cfg.last_tested = datetime.utcnow()
        cfg.updated_at  = datetime.utcnow()
        db.session.commit()

        return jsonify({'success': success, 'message': message, 'config': cfg.to_safe_dict()})
    except Exception as e:
        logger.error(f"smb_test_connection error: {e}", exc_info=True)
        return jsonify({'success': False, 'message': str(e)}), 500


@storage_bp.route('/api/storage/smb/files')
@login_required
def smb_list_files():
    if not current_user.is_admin():
        return jsonify({'error': 'Unauthorized'}), 403
    try:
        from models import SMBConfig
        from smb_helper import list_smb_backups, _fmt_bytes
        cfg = SMBConfig.get_instance()
        if not cfg.server_path:
            return jsonify({'error': 'SMB network share is not configured.'}), 400
        if not cfg.is_verified:
            return jsonify({'error': 'SMB share has not been verified. Please test the connection first.'}), 400
        files = list_smb_backups(cfg.server_path, cfg.username, cfg.password, cfg.domain or '')
        return jsonify({'files': files})
    except Exception as e:
        logger.error(f"smb_list_files error: {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500


@storage_bp.route('/api/storage/smb/restore', methods=['POST'])
@login_required
def smb_restore_file():
    """Download a backup from the SMB share and restore it to OpenSearch."""
    if not current_user.is_admin():
        return jsonify({'error': 'Unauthorized'}), 403
    try:
        from models import SMBConfig, LogBackup, db
        from smb_helper import _parse_unc, download_from_smb
        data      = request.json or {}
        unc_path  = (data.get('unc_path') or '').strip()
        file_name = (data.get('file_name') or '').strip()
        if not unc_path or not file_name:
            return jsonify({'error': 'unc_path and file_name are required.'}), 400

        cfg = SMBConfig.get_instance()
        if not cfg.server_path:
            return jsonify({'error': 'SMB network share not configured.'}), 400

        server, _ = _parse_unc(cfg.server_path)

        # Create a DB record for this restore
        backup = LogBackup(
            name=f'smb-restore-{file_name}',
            log_type='unknown',
            file_path=unc_path,
            destination_type='smb',
            status='restoring',
            notes=f'Restored from SMB: {unc_path}',
            created_by=current_user.id,
        )
        db.session.add(backup)
        db.session.commit()
        ...
        def _run_smb_restore():
            ...
            with _app.app_context():
                ...
                tmp_path = None
                try:
                    with tempfile.NamedTemporaryFile(suffix='.ndjson.gz', delete=False, dir=_DEFAULT_BACKUP_DIR) as tmp:
                        tmp_path = tmp.name
                    download_from_smb(unc_path, tmp_path, server, cfg.username, cfg.password, cfg.domain or '')
                    api = OpenSearchAPI()
                    def _progress(n):
                        with _job_lock:
                            _job_progress[backup_id] = n
                    success, failed = api.restore_ndjson_to_opensearch(tmp_path, progress_cb=_progress)
                    rec.status       = 'restored'
                    rec.doc_count    = success
                    rec.completed_at = datetime.utcnow()
                    logger.info(f"SMB restore {backup_id} complete — {success} docs")
                except Exception as err:
                    rec.status        = 'failed'
                    rec.error_message = str(err)
                    logger.error(f"SMB restore {backup_id} failed: {err}")
                finally:
                    _db.session.commit()
                    with _job_lock:
                        _job_progress.pop(backup_id, None)
        ...
        import threading
        threading.Thread(target=_run_smb_restore, daemon=True).start()
        return jsonify({'success': True, 'backup_id': backup_id,
                        'message': 'SMB restore started in background.'})
    except Exception as e:
        logger.error(f"smb_restore_file error: {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500
