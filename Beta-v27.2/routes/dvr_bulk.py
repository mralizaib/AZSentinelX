...
COLUMN_ALIASES = {
    ...
    # manager account
    'manager user': 'manager_username', 'manager username': 'manager_username',
    'manager pass': 'manager_password', 'manager pwd': 'manager_password',
    'manager password': 'manager_password', 'create manager': 'create_manager',
    # use_https
    'https': 'use_https', 'ssl': 'use_https',
    # admin_verification_password
    'admin verif password': 'admin_verification_password',
    'admin verif pass': 'admin_verification_password',
    'verification password': 'admin_verification_password',
    'verif password': 'admin_verification_password',
    'admin_verif_password': 'admin_verification_password',
    'secondary password': 'admin_verification_password',
    'secondary pass': 'admin_verification_password',
}
...
def _configure_one_device(app, job_id, device_idx, device, global_city, global_state):
    ...
    try:
        client = DVRClient(
            ...
            admin_verification_password=device.get('admin_verification_password') or None,
        )
        ...
        for main_id in channel_ids:
            ...
            event_res = client.configure_event_stream(main_id)
            stream_results.append({
                'channel':          ch_label,
                'main':             main_res['status'],
                ...
                'event':            event_res['status'],
                'event_resolution': event_res['applied_resolution'],
                'event_bitrate':    event_res['applied_bitrate'],
                'event_note':       event_res['note'],
                'online':           True,
            })
        ...
        # Manager account is read from the imported file (CSV/Excel/DB) on a
        # per-device basis: 'manager_username' / 'manager_password' columns,
        # or an explicit 'create_manager' flag to use the defaults below. If
        # no credentials and no flag are present for this device, the manager
        # step is skipped for that device — not an error, not a failure.
        manager_username_raw = str(device.get('manager_username') or '').strip()
        manager_password_raw = str(device.get('manager_password') or '').strip()
        _cm_raw = device.get('create_manager', False)
        create_manager_flag = _cm_raw if isinstance(_cm_raw, bool) else str(_cm_raw).lower() in ('true', 'yes', '1')
        create_manager = create_manager_flag or bool(manager_username_raw) or bool(manager_password_raw)
        ...
        from routes.dvr_config import _compute_overall
...
def _build_bulk_html_summary(job_data, job):
    """Build a professional PDF-ready bulk report with per-device detail sections."""
    devices = job_data.get('devices', [])
    total   = len(devices)
    n_pass  = sum(1 for d in devices if d.get('overall') == 'PASS')
    n_warn  = sum(1 for d in devices if d.get('overall') == 'WARNING')
    n_part  = sum(1 for d in devices if d.get('overall') == 'PARTIAL SUCCESS')
    n_fail  = sum(1 for d in devices if d.get('overall') == 'FAIL')
    ...
    for i, d in enumerate(devices, 1):
        overall = d.get('overall', 'N/A')
        bg      = OVERALL_BG.get(overall, '#555')
        icon    = OVERALL_ICON.get(overall, '?')
        val     = d.get('steps', {}).get('validation', {})
        err     = (f"<div style='color:#c0392b;font-size:.82em;margin-top:3px;'>"
                   f"⚠ {_h(d.get('error',''))}</div>") if d.get('error') else ''
        summary_rows += f"""<tr>
          <td style="font-weight:600;">{i}</td>
          <td><a href="#device-{i}" style="color:#0056b3;text-decoration:none;font-weight:600;">{_h(d['device_name'])}</a>{err}</td>
          <td><code>{_h(d['ip'])}:{_h(d.get('port', 80))}</code></td>
          <td><span style="background:{bg};color:#fff;padding:3px 10px;border-radius:4px;font-weight:600;font-size:.88em;">{icon} {_h(overall)}</span></td>
          <td style="white-space:nowrap;">
            <span style="color:#1a7f4b;">✅ {val.get('passed',0)}</span> &nbsp;
            <span style="color:#b07800;">⚠️ {val.get('warned',0)}</span> &nbsp;
            <span style="color:#c0392b;">❌ {val.get('failed',0)}</span>
          </td>
          <td style="color:#6c757d;font-size:.88em;">{_h(d.get('status',''))}</td>
        </tr>"""
    ...
    for i, d in enumerate(devices, 1):
        ...
        checks   = val.get('checks', [])
        vp, vw, vf = val.get('passed', 0), val.get('warned', 0), val.get('failed', 0)

        # info grid
        info_grid = f"""<div class="info-grid">
          <div class="info-cell"><label>Device Name</label><span>{_h(d.get('device_name','N/A'))}</span></div>
          <div class="info-cell"><label>IP Address</label><span><code>{_h(d['ip'])}:{_h(d.get('port',80))}</code></span></div>
          <div class="info-cell"><label>Manufacturer</label><span>{_h(info.get('manufacturer','N/A'))}</span></div>
          <div class="info-cell"><label>Model</label><span>{_h(info.get('model','N/A'))}</span></div>
          <div class="info-cell"><label>Device Type</label><span>{_h(info.get('device_type','N/A'))}</span></div>
          <div class="info-cell"><label>Serial Number</label><span>{_h(info.get('serial_number','N/A'))}</span></div>
          <div class="info-cell"><label>Firmware Version</label><span>{_h(info.get('firmware_version','N/A'))}</span></div>
          <div class="info-cell"><label>MAC Address</label><span>{_h(info.get('mac_address','N/A'))}</span></div>
          <div class="info-cell"><label>Total / Active Channels</label><span>{_h(info.get('total_channels','N/A'))} / {_h(info.get('active_channels','N/A'))}</span></div>
          <div class="info-cell"><label>Location</label><span>{_h(time_cfg.get('city',''))}, {_h(time_cfg.get('state',''))}</span></div>
          <div class="info-cell"><label>Timezone</label><span>{_h(time_cfg.get('iana_timezone','N/A'))}</span></div>
          <div class="info-cell"><label>NTP Server</label><span>{_h(time_cfg.get('ntp_server','pool.ntp.org'))}</span></div>
          <div class="info-cell"><label>Completed At</label><span>{_h((d.get('completed_at','') or '')[:19])}</span></div>
        </div>"""
        ...
        for h in hdds:
            ...
            ico = '✅' if ok else '❌'
            hdd_rows += (f"<tr><td>{_h(h.get('id',''))}</td><td>{_h(h.get('name',''))}</td>"
                         f"<td>{_h(h.get('capacity',''))} MB</td>"
                         f"<td style='color:{c};font-weight:600;'>{ico} {_h(h.get('status',''))}</td>"
                         f"<td>{_h(h.get('free_space',''))} MB</td><td>{_h(h.get('property',''))}</td></tr>")
        if not hdd_rows:
            hdd_rows = '<tr><td colspan="6" style="color:#999;text-align:center;">No HDD data available</td></tr>'
        ...
        for u in users:
            ...
            c   = '#1a7f4b' if ok else '#c0392b'
            ico = '✅' if ok else '❌'
            user_rows += (f"<tr><td><strong>{_h(u.get('username',''))}</strong></td>"
                          f"<td><code>{_h(u.get('password','N/A'))}</code></td>"
                          f"<td>{_h(u.get('role','N/A'))}</td>"
                          f"<td style='color:{c};font-weight:600;'>{ico} {_h(u.get('status','N/A'))}</td></tr>")
        if not user_rows:
            user_rows = '<tr><td colspan="4" style="color:#999;text-align:center;">No user data available</td></tr>'
        ...
        for c in checks:
            ...
            bg_row = ST_BG.get(c['status'], '#fff')
            action = (f'<div class="action-note">⟶ {_h(c.get("action",""))}</div>'
                      if c.get('action') else '')
            check_rows += (f'<tr style="background:{bg_row};">'
                           f'<td style="color:{color};font-size:1.1em;text-align:center;width:36px;">{icon}</td>'
                           f'<td style="font-weight:600;white-space:nowrap;">{_h(c["check"])}</td>'
                           f'<td>{_h(c["detail"])}{action}</td></tr>')
        if not check_rows:
            check_rows = '<tr><td colspan="3" style="color:#999;">No validation data available</td></tr>'

        detail_sections += f"""
        <div id="device-{i}" class="device-section">
          <div class="device-header" style="background:{hdr_bg};">
            <div class="device-header-left">
              <div class="device-num">Device {i}</div>
              <div class="device-name">{_h(d.get('device_name',''))} &nbsp;<span style="opacity:.75;font-size:.7em;font-weight:400;">{_h(d['ip'])}</span></div>
            </div>
            <div class="device-scores">
              <span class="dscore">{hdr_icon} {_h(overall)}</span>
              <span class="dscore-sm">✅ {vp} &nbsp; ⚠️ {vw} &nbsp; ❌ {vf}</span>
            </div>
          </div>
          <div class="device-body">
            ...
            <div class="section-title" style="margin-top:18px;">Configuration Validation</div>
            <table>
              <tr><th style="width:36px;">St.</th><th style="width:220px;">Check</th><th>Detail / Action</th></tr>
              {check_rows}
            </table>

            <div class="section-title" style="margin-top:18px;">Storage — HDD Information</div>
            <table>
              <tr><th>ID</th><th>Name</th><th>Capacity</th><th>Status</th><th>Free Space</th><th>Property</th></tr>
              {hdd_rows}
            </table>

            <div class="section-title" style="margin-top:18px;">User Accounts</div>
            <table>
...
<head>
<meta charset="UTF-8">
<title>REBIZ Sentinel X — Bulk Configuration Report</title>
<style>
*{{box-sizing:border-box;margin:0;padding:0;}}
body{{font-family:'Segoe UI',Arial,sans-serif;font-size:13px;color:#1a1a2e;background:#f4f6f9;}}
.page{{max-width:980px;margin:0 auto;background:#fff;box-shadow:0 2px 16px rgba(0,0,0,.12);}}

.report-header{{background:#1a1a2e;color:#fff;padding:28px 36px 22px;}}
.report-header .brand{{font-size:1em;opacity:.75;letter-spacing:.06em;text-transform:uppercase;margin-bottom:4px;}}
.report-header h1{{font-size:1.6em;font-weight:700;}}
.report-header .meta{{font-size:.86em;opacity:.8;margin-top:8px;}}
...
.score-card .lbl{{font-size:.76em;text-transform:uppercase;letter-spacing:.05em;color:#6c757d;margin-top:2px;}}
.sc-total .num{{color:#0d6eac;}} .sc-pass .num{{color:#1a7f4b;}} .sc-part .num{{color:#0d6eac;}}
.sc-warn .num{{color:#b07800;}} .sc-fail .num{{color:#c0392b;}}

.section{{padding:22px 36px;border-bottom:1px solid #e8ecf0;}}
.section-title{{font-size:.88em;font-weight:700;text-transform:uppercase;letter-spacing:.07em;
               color:#0056b3;border-left:4px solid #0056b3;padding-left:10px;margin-bottom:12px;}}

table{{width:100%;border-collapse:collapse;font-size:.88em;}}
th{{background:#0056b3;color:#fff;padding:8px 10px;text-align:left;font-weight:600;font-size:.8em;text-transform:uppercase;letter-spacing:.04em;}}
td{{padding:7px 10px;border-bottom:1px solid #edf0f5;vertical-align:top;}}
tr:last-child td{{border-bottom:none;}}
.action-note{{font-size:.8em;color:#8a6200;margin-top:3px;background:#fffbea;padding:2px 6px;border-radius:3px;border-left:3px solid #ffc107;}}

.info-grid{{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;}}
.info-cell{{background:#f9fbff;border:1px solid #dce4f0;border-radius:5px;padding:8px 11px;}}
.info-cell label{{font-size:.72em;color:#6c757d;display:block;text-transform:uppercase;letter-spacing:.04em;margin-bottom:2px;}}
.info-cell span{{font-size:.9em;font-weight:600;color:#1a1a2e;word-break:break-all;}}

.device-section{{border:1px solid #dce4f0;border-radius:8px;margin:20px 36px;overflow:hidden;}}
.device-header{{color:#fff;padding:14px 20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;}}
.device-num{{font-size:.8em;opacity:.8;text-transform:uppercase;letter-spacing:.08em;}}
.device-name{{font-size:1.15em;font-weight:700;}}
.device-scores{{text-align:right;}}
.dscore{{display:block;font-size:1.1em;font-weight:700;}}
.dscore-sm{{display:block;font-size:.82em;opacity:.9;margin-top:2px;}}
.device-body{{padding:18px 20px;}}

code{{background:#f0f4ff;color:#0d3c7a;padding:2px 5px;border-radius:3px;font-size:.88em;}}
a{{color:#0056b3;}}
.report-footer{{background:#1a1a2e;color:#aab;padding:12px 36px;text-align:center;font-size:.8em;}}
@media print{{.device-section{{page-break-inside:avoid;margin:10px 0;}} body{{background:#fff;}} .page{{box-shadow:none;}}}}
</style>
</head>
<body>
<div class="page">
...
<div class="report-header">
  <div class="brand">REBIZ Sentinel X &mdash; DVR/NVR Bulk AutoDeploy</div>
  <h1>Bulk Configuration Report</h1>
  <div class="meta">📅 Generated: {ts} UTC &nbsp;·&nbsp; 🔑 Job ID: <code style="background:rgba(255,255,255,.15);color:#fff;">{_h(job.id)}</code></div>
</div>

<div class="score-bar">
  <div class="score-card sc-total"><div class="num">{total}</div><div class="lbl">Total Devices</div></div>
  <div class="score-card sc-pass"><div class="num">{n_pass}</div><div class="lbl">✅ Pass</div></div>
  <div class="score-card sc-part"><div class="num">{n_part}</div><div class="lbl">🟡 Partial</div></div>
  <div class="score-card sc-warn"><div class="num">{n_warn}</div><div class="lbl">⚠️ Warning</div></div>
  <div class="score-card sc-fail"><div class="num">{n_fail}</div><div class="lbl">❌ Failed</div></div>
</div>

<div class="section">
  <div class="section-title">Deployment Summary</div>
  <table>
    <tr><th>#</th><th>Device Name</th><th>IP : Port</th><th>Overall Result</th><th>Validation Counts</th><th>Status</th></tr>
    {summary_rows}
  </table>
</div>

<div class="section">
  <div class="section-title">Device Detail Reports</div>
</div>

{detail_sections}

<div class="report-footer">
  REBIZ Sentinel X &mdash; DVR/NVR Bulk AutoDeploy &nbsp;|&nbsp; {ts} UTC
</div>
...
<hr><p style="color:#999;font-size:0.85em;">REBIZ Sentinel X — AI DVR/NVR Bulk Auto-Configuration</p>
</body></html>"""
...
def download_template(fmt):
    ...
    cols = ['device_name', 'ip', 'port', 'username', 'password',
            'admin_verification_password',
            'city', 'state', 'client_initials', 'dlt_password', 'cms_password',
            'manager_username', 'manager_password', 'create_manager', 'use_https']
    ...
    if fmt == 'csv':
        ...
        instructions = [
            ...
            ('password',    'Required', 'Admin login password.'),
            ('admin_verification_password', 'Optional', 'Secondary admin password required by some firmware variants for Double Verification during user account creation. Leave blank if the device does not require a second admin confirmation.'),
            ...
            ('manager_username', 'Optional', 'Manager account username. Defaults to "manager" if manager_password or create_manager is set.'),
            ('manager_password', 'Optional', 'Manager account password. Defaults to <client_initials>man_12. Leave both manager fields and create_manager blank to skip the manager account entirely for this device.'),
            ('create_manager',   'Optional', 'true/yes/1 to create the manager account using the defaults above without specifying credentials.'),
...
def upload_devices():
    ...
    for d in devices:
        preview.append({
            ...
            'has_admin_verification_password': bool(d.get('admin_verification_password')),
            'manager_username': d.get('manager_username', ''),
            'has_manager_password': bool(d.get('manager_password')),
            'create_manager': bool(d.get('manager_username')) or bool(d.get('manager_password'))
                              or str(d.get('create_manager', '')).lower() in ('true', 'yes', '1'),
