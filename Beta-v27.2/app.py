...
# Every login sets session.permanent = True (see routes/auth.py), so the
# session uses a sliding idle-timeout: PERMANENT_SESSION_LIFETIME is the max
# gap allowed between requests, and it's renewed on every authenticated
...
def before_request():
    ...
    if current_user.is_authenticated:
        ...
        # session.permanent is always set at login time (see routes/auth.py),
        # so this renews the sliding idle-timeout window on every request.
