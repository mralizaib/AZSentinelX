/**
 * Dashboard JavaScript - REBIZ Sentinel X
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.body.addEventListener('click', function(e) {
        ...
        if (activityRow) {
            ...
            // Using search filter pattern to show details in current page if possible,
            // or redirecting to dedicated view page
        ...
        if (ruleRow) {
            ...
            const ruleDesc = ruleRow.getAttribute('data-rule-desc') || '';
            // Navigate to alerts page filtered by this specific rule ID
            window.location.href = `/alerts?rule_id=${encodeURIComponent(ruleId)}&rule_desc=${encodeURIComponent(ruleDesc)}&severity=all`;
            return;
        }
    });
...
if (typeof agentDetailsModal === 'undefined') {
    window.agentDetailsModal = null;
}
    ...
    fetch(`/api/dashboard/top_rules?days=${days}`)
        ...
        .then(rules => {
            ...
            rules.forEach(rule => {
                ...
                row.setAttribute('data-rule-desc', rule.description);
                row.innerHTML = `
                    <td class="ps-4 py-3">
                        ...
                        <div class="small text-muted">Rule ID: ${rule.rule_id} | Level: ${rule.level}
                            <span class="ms-2 text-info" style="font-size:10px;"><i class="fas fa-arrow-right me-1"></i>Click to view alerts</span>
                        </div>
                    </td>
                `;
                tableBody.appendChild(row);
...
function showAgentDetails(status = 'all') {
    ...
    if (modalInstance) {
        modalInstance.hide();
        // Give it a small timeout to finish hiding before disposing
        setTimeout(() => {
    ...
    function initializeAndShowModal() {
        // Dispose any existing backdrop that might be stuck
...
function exportAgentsToExcel() {
    if (!window.currentAgents) return;
    const header = 'ID,Name,IP Address,Status,Group,Location,OS,Last Keep Alive';
    const rows = window.currentAgents.map(a => {
        const groups = Array.isArray(a.group) ? a.group.join('; ') : (a.group || '');
