...
def _build_channel_check_result(check_name, all_channel_ids, ok_ids, fixed_ids,
                                  ...
                                  warn_ids=None, severity="major", critical=False):
    ...
    warn_ids : channels where a fallback/degraded setting was applied (status=warn).
    severity : 'critical' | 'major' | 'minor'  — drives overall status computation.
    critical : convenience alias for severity='critical'.
    """
    ...
    if not fail_ids and not warn_ids and ok_ids:
        ...
        if fixed_ids:
            ...
            detail += "."
        status = "pass"
    elif not fail_ids and warn_ids:
        # Fallback/degraded settings applied — warn, not fail
        detail = f"Applied with fallback settings on channel(s) {warn_labels}."
        if ok_ids:
            detail += f" Primary settings confirmed on channel(s) {ok_labels}."
        if target_desc:
            detail += f" Target: {target_desc}"
        status = "warn"
    elif ok_ids or warn_ids:
        good_labels = ok_labels or warn_labels
        detail = f"Verified/applied on channel(s) {good_labels}. Failed on channel(s) {fail_labels}."
        status = "warn"
    else:
    ...
    return {
        ...
        "action": "" if status == "pass" else
                  f"Manually verify {check_name.lower()} on channel(s) {fail_labels or warn_labels or skip_labels}.",
    }
...
<storage>
  <hddList>
    ...
    def configure_main_stream(self, channel_id):
        """Configure main stream with intelligent resolution and bitrate fallback.

        Resolution chain (highest first, never exceeds 1920×1080):
            1920×1080 → 1280×720 → 960×540 → 704×576 → 640×480

        Bitrate chain per resolution attempt (kbps):
            320 → 512 → 384 → 768 → 1024

        For each resolution the bitrate chain is tried in full before moving to
        the next lower resolution, so a device that rejects 1920×1080 at 320 kbps
        but accepts it at 512 kbps will still be configured at 1920×1080.

        Returns a dict:
          {
            'ok':                 bool,
            'status':             'configured' | 'configured_fallback' | 'error',
            'applied_resolution': '1920×1080' | '<WxH>' | 'N/A',
            'applied_bitrate':    '320' | '<kbps>' | 'N/A',
            'note':               str,
          }
        """
        RESOLUTION_CHAIN = [
        ...
        BITRATE_CHAIN = [320, 512, 384, 768, 1024]
        PRIMARY_W, PRIMARY_H, PRIMARY_BR = "1920", "1080", 320
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                return {'ok': False, 'status': 'error',
                        'applied_resolution': 'N/A', 'applied_bitrate': 'N/A',
                        'note': f'GET returned HTTP {r.status_code}'}
            original_xml = r.text
            last_error = None

            for w, h in RESOLUTION_CHAIN:
                for br in BITRATE_CHAIN:
                    try:
                        patched = _patch_xml_fields(original_xml, [
                            ...
                            ("maxFrameRate",            "800"),
                            ("vbrUpperCap",             str(br)),
                            ("constantBitRate",         str(br)),
                        ])
                        self._put(f"/Streaming/channels/{channel_id}", patched)
                        is_primary = (w == PRIMARY_W and h == PRIMARY_H and br == PRIMARY_BR)
                        res_str    = f"{w}×{h}"
                        status     = 'configured' if is_primary else 'configured_fallback'
                        notes = []
                        if w != PRIMARY_W or h != PRIMARY_H:
                            notes.append(f"1920×1080 not supported — device accepted {res_str}")
                        if br != PRIMARY_BR:
                            notes.append(f"320 kbps not supported — device accepted {br} kbps")
                        note = "; ".join(notes)
                        logger.info(
                            f"Main stream {channel_id}: configured {res_str} H.264 CBR {br}kbps 8fps"
                            + (" (fallback)" if not is_primary else "")
                        )
                        return {'ok': True, 'status': status,
                                'applied_resolution': res_str,
                                'applied_bitrate': str(br),
                                'note': note}
                    except Exception as e_combo:
                        last_error = e_combo
                        logger.debug(
                            f"Main stream {channel_id}: {w}×{h} @ {br}kbps rejected ({e_combo}); "
                            f"trying next combination…"
                        )
                        continue

            logger.warning(
                f"Main stream {channel_id}: all resolution/bitrate combinations rejected. "
                f"Last error: {last_error}"
            )
            return {'ok': False, 'status': 'error',
                    'applied_resolution': 'N/A', 'applied_bitrate': 'N/A',
                    'note': f'All combinations rejected. Last: {last_error}'}
        except Exception as e:
            logger.warning(f"Main stream config channel {channel_id}: {e}")
            return {'ok': False, 'status': 'error',
                    'applied_resolution': 'N/A', 'applied_bitrate': 'N/A', 'note': str(e)}

    def configure_event_stream(self, channel_id):
        """Configure main-stream event channel (ID suffix '03') with the same
        intelligent resolution and bitrate fallback as configure_main_stream.

        Resolution chain: 1920×1080 → 1280×720 → 960×540 → 704×576 → 640×480
        Bitrate chain (kbps): 320 → 512 → 384 → 768 → 1024

        Returns a dict:
          {
            'ok':                 bool,
            'status':             'configured' | 'configured_fallback' | 'skipped' | 'error',
            'applied_resolution': '1920×1080' | '<WxH>' | 'N/A',
            'applied_bitrate':    '320' | '<kbps>' | 'N/A',
            'note':               str,
          }
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                return {'ok': False, 'status': 'skipped',
                        'applied_resolution': 'N/A', 'applied_bitrate': 'N/A',
                        'note': f'Channel not available (HTTP {r.status_code})'}
            original_xml = r.text
            last_error = None
            ...
            for w, h in RESOLUTION_CHAIN:
                for br in BITRATE_CHAIN:
                    try:
                        ...
                        note = "; ".join(notes)
                        logger.info(
                            f"Event stream {event_id}: configured {res_str} H.264 CBR {br}kbps 8fps"
                            + (" (fallback)" if not is_primary else "")
                        )
                        return {'ok': True, 'status': status,
                                'applied_resolution': res_str,
                        ...
                        last_error = e_combo
                        logger.debug(
                            f"Event stream {event_id}: {w}×{h} @ {br}kbps rejected ({e_combo}); "
                            f"trying next combination…"
                        )
                        continue

            logger.warning(
                f"Event stream {event_id}: all resolution/bitrate combinations rejected. "
                f"Last error: {last_error}"
            )
            return {'ok': False, 'status': 'error',
                    'applied_resolution': 'N/A', 'applied_bitrate': 'N/A',
    ...
    def configure_sub_stream(self, channel_id):
        """Configure sub stream with intelligent resolution and bitrate fallback.

        Resolution chain (primary first):
            960×576 → 960×540 → 704×576 → 704×480 → 640×480
        ...
        BITRATE_CHAIN = [320, 512, 384, 768, 1024]
        PRIMARY_W, PRIMARY_H, PRIMARY_BR = "960", "576", 320
            ...
            for w, h in RESOLUTION_CHAIN:
                for br in BITRATE_CHAIN:
                    try:
                        ...
                        notes = []
                        if w != PRIMARY_W or h != PRIMARY_H:
                            notes.append(f"960×576 not supported — device accepted {res_str}")
                        if br != PRIMARY_BR:
                            notes.append(f"320 kbps not supported — device accepted {br} kbps")
                        note = "; ".join(notes)
                        logger.info(
                            f"Sub stream {sub_id}: configured {res_str} H.264 CBR {br}kbps 8fps"
                            + (" (fallback)" if not is_primary else "")
                        )
                        return {'ok': True, 'status': status,
                                'applied_resolution': res_str,
                        ...
                        last_error = e_combo
                        logger.debug(
                            f"Sub stream {sub_id}: {w}×{h} @ {br}kbps rejected ({e_combo}); "
                            f"trying next combination…"
                        )
                        continue

            logger.warning(
                f"Sub stream {sub_id}: all resolution/bitrate combinations rejected. "
                f"Last error: {last_error}"
            )
            return {'ok': False, 'status': 'error',
                    'applied_resolution': 'N/A', 'applied_bitrate': 'N/A',
  ...
  <localPermission>{local_perms}
    ...
    def validate_configuration(self, expected):
        ...
        try:
            ...
            results.append({"check": "Time/NTP", "status": "fail", "severity": "critical",
        ...
        try:
            ...
            results.append({"check": "Device Name", "status": "warn", "severity": "minor",
        ...
        try:
            ...
            results.append({"check": "HDD Status", "status": "fail", "severity": "major",
        ...
        # Main stream — verify every online channel; track fallback channels as warnings
        _main_pass, _main_fixed, _main_fail, _main_warn = [], [], [], []
        for _cid in _online_channel_ids:
            try:
                ...
                if _ok:
                    ...
                    _res = self.configure_main_stream(_cid)
                    if _res.get('ok'):
                        if _res.get('status') == 'configured_fallback':
                            _main_warn.append(_cid)  # fallback used — warn, not pass/fail
                        else:
        ...
        # Sub stream — verify every online channel; fallback resolutions → warn
        _sub_pass, _sub_fixed, _sub_fail, _sub_warn = [], [], [], []
        for _cid in _online_channel_ids:
            ...
            try:
                ...
                if _sub_res.get("ok"):
                    if _sub_res.get('status') == 'configured_fallback':
                        _sub_warn.append(_cid)  # fallback used — warn, not pass/fail
                    else:
        ...
        try:
            ...
            results.append({"check": "Channel Zero Status", "status": "warn", "severity": "minor",
