...
def configure_streams():
    ...
    try:
        ...
        all_channel_ids = discovery.get('channel_ids') or []
        if not all_channel_ids:
            total_channels = int(discovery.get('total_channels', 0)) or 4
            all_channel_ids = [f"{ch}01" for ch in range(1, total_channels + 1)]
        ...
        for main_id in channel_ids:
            ...
            results.append({
                ...
                'event_stream': 'configured' if event_ok else 'skipped',
                'online': True,
            })

        # Record skipped offline channels so the report reflects them
...
def configure_recording():
    ...
    try:
        ...
        # Only apply recording schedule to active/online channels.
        channel_ids = client.get_online_channel_ids(all_channel_ids)
        skipped_ids = [cid for cid in all_channel_ids if cid not in channel_ids]
        ...
        for main_id in channel_ids:
            ...
            results.append({'channel': ch_label, 'status': 'configured' if ok else 'error', 'online': True})

        for cid in skipped_ids:
            ch_label = cid[:-2] if len(cid) > 2 else cid
            results.append({'channel': ch_label, 'status': 'skipped', 'online': False})
