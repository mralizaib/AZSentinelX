...
OPTIONAL_COLS = {
    ...
    'port': '80',
    'use_https': 'false',
    'timezone': '',   # preferred: IANA name e.g. "America/Chicago"
    'city': '',       # legacy fallback — still accepted for old sheets
    'state': '',      # legacy fallback — US state name
    'client_initials': '',
    'dlt_password': '',
    'cms_password': '',
    'manager_username': '',
...
COLUMN_ALIASES = {
    ...
    # timezone (new preferred column)
    'time zone': 'timezone', 'tz': 'timezone', 'iana': 'timezone', 'iana_tz': 'timezone',
    # city / state (legacy — still accepted for old spreadsheets)
...
def _configure_one_device(app, job_id, device_idx, device, global_timezone='', global_city='', global_state=''):
    """Run all 9 config steps for a single device. Updates job data in DB."""
    from dvr_api import DVRClient, TZ_TO_HIKVISION
    ...
    #   1. Per-device 'timezone' column (IANA name, e.g. "America/Chicago")
    #   2. Global timezone fallback from the UI
    #   3. Legacy per-device city/state columns (backward compat)
    #   4. Legacy global city/state (backward compat)
    iana_timezone = device.get('timezone') or global_timezone or None
    # validate — ignore unknown IANA names so state fallback still works
    if iana_timezone and iana_timezone not in TZ_TO_HIKVISION:
        logger.warning(f"Unknown timezone '{iana_timezone}' for {device['ip']}; falling back to state lookup")
        iana_timezone = None
    # Legacy fallbacks
    city  = device.get('city')  or global_city  or ''
    ...
    try:
        ...
        _update_job()
        time_cfg = client.configure_time(city, state, iana_timezone=iana_timezone)
        ...
        recording_track_ids = client.get_native_recording_track_ids()
        ...
        result['steps']['streams'] = {
            'status': 'done' if all(
                row['main'] in ('ok', 'skipped')
                and row['sub'] in ('configured', 'configured_fallback', 'skipped')
        ...
        _update_job()
        rec_results = []
        for track_id in (recording_track_ids or channel_ids):
            ch_label = track_id[:-2] if len(track_id) > 2 else track_id
            ok = client.configure_recording_schedule(track_id)
        ...
        result['steps']['recording'] = {
            'status': 'done' if all(r['status'] in ('ok', 'skipped') for r in rec_results) else 'partial',
        ...
        _update_job()
        holiday_schedule_ok = client.configure_holiday()
        holiday_recording = []
        for track_id in (recording_track_ids or channel_ids):
            holiday_recording.append({
                'channel': track_id[:-2] if len(track_id) > 2 else track_id,
                **client.configure_holiday_recording(track_id),
            })
        holiday_recording_ok = bool(holiday_recording) and all(
            entry['ok'] for entry in holiday_recording
        )
        result['steps']['holiday'] = {
            'status': 'done' if holiday_schedule_ok and holiday_recording_ok else 'partial',
            'holiday_schedule': holiday_schedule_ok,
            'holiday_recording': holiday_recording,
        }
        ...
        result['steps']['users'] = {
            'status': 'done' if all(
                u.get('status') not in ('error', 'created_permissions_failed',
                                        'updated_permissions_failed')
                for u in user_results
            ) else 'partial', 'accounts': user_results,
        ...
        expected['recording_track_ids'] = recording_track_ids
...
def _run_bulk_job(app, job_id, devices, global_timezone, recipients, email_formats, global_city='', global_state=''):
    ...
    for idx, device in enumerate(devices):
        res = _configure_one_device(app, job_id, idx, device, global_timezone=global_timezone, global_city=global_city, global_state=global_state)
        if res['status'] == 'done':
            succeeded += 1
        else:
            failed_count += 1
...
def download_template(fmt):
    ...
    if fmt == 'csv':
        ...
        instructions = [
            ...
            ('timezone',     'Optional', 'IANA timezone name for this device (e.g. America/Chicago, Asia/Karachi, Europe/London). Takes priority over city/state. Falls back to global timezone setting.'),
            ('city',        'Optional', 'Legacy: city name (ignored by device — only used to infer US state). Use the timezone column instead.'),
            ('state',       'Optional', 'Legacy: US state name for timezone lookup (e.g. Texas). Ignored when timezone column is present.'),
...
def upload_devices():
    ...
    for d in devices:
        preview.append({
            ...
            'city': d.get('city', ''),    # legacy
            'state': d.get('state', ''), # legacy
...
def start_bulk():
    ...
    Expects multipart/form-data with:
      - file: CSV or Excel
      - global_timezone: IANA fallback timezone (e.g. "America/Chicago")
      - global_city, global_state: legacy fallback (still accepted)
    ...
    global_timezone = request.form.get('global_timezone', '').strip()
    global_city     = request.form.get('global_city', '').strip()   # legacy
    global_state    = request.form.get('global_state', '').strip()  # legacy
    ...
    job.set_data({'devices': [], 'global_timezone': global_timezone})
    db.session.add(job)
    db.session.commit()

    t = threading.Thread(
        target=_run_bulk_job,
        args=(app, job_id, devices, global_timezone, recipients, email_formats),
        kwargs={'global_city': global_city, 'global_state': global_state},
        daemon=True,
    )
    t.start()
