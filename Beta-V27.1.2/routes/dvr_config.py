...
def connect():
    ...
    required = ['device_name', 'url', 'username', 'password']
    for f in required:
        if not data.get(f):
    ...
    try:
        ...
        if not ok:
            return jsonify({'success': False,
                            'error': 'Authentication failed. Check the DVR/NVR URL, username and password.'}), 401
        ...
        if not name_applied:
            logger.warning(f"Could not set device name on {data['url']}; continuing anyway")
...
def configure_time():
    data = request.get_json() or {}
    sess_id = data.get('session_id')
    # New: accept iana_timezone directly.  Legacy city/state kept for
    # backward compat with any scripts that POST the old fields.
    iana_timezone = data.get('iana_timezone', '').strip()
    city  = data.get('city', '')
    ...
    try:
        client = _make_client(sess['connection'])
        result = client.configure_time(city, state, iana_timezone=iana_timezone or None)
        # Expose the resolved IANA name in the response so the UI can display it.
        result.setdefault('iana_timezone', iana_timezone)
...
def configure_streams():
    ...
    try:
        ...
        verified = bool(results) and all(
            row['main_stream'] == 'configured'
            and row['sub_stream'] in ('configured', 'configured_fallback')
            for row in results
        )
        sess['steps']['streams'] = {
            'status': 'done' if verified else 'partial',
            'channels': results,
            'channel_zero_disabled': True,
        }
        _save_session_data(sess_id, sess)

        return jsonify({'success': True, 'verified': verified, 'channels': results})
    except Exception as e:
...
def configure_recording():
    ...
    try:
        client = _make_client(sess['connection'])
        discovery = sess['steps'].get('discovery', {})
        channel_ids = client.get_native_recording_track_ids()
        if not channel_ids:
            channel_ids = discovery.get('channel_ids') or []
        ...
        verified = bool(results) and all(
            row['status'] == 'configured' for row in results
        )
        sess['steps']['recording'] = {
            'status': 'done' if verified else 'partial',
            'channels': results,
        }
        _save_session_data(sess_id, sess)
    ...
    try:
        ...
        for main_id in channel_ids:
            holiday_recording.append({
                'channel': main_id[:-2] if len(main_id) > 2 else main_id,
                **client.configure_holiday_recording(main_id),
            })
        holiday_recording_verified = bool(holiday_recording) and all(
            row['ok'] for row in holiday_recording
        )
        ...
        holiday_info = {
            ...
            'status': 'configured' if ok and holiday_recording_verified else 'warn',
            'holiday_schedule_verified': ok,
        ...
        return jsonify({
            'success': True,
            'verified': ok and holiday_recording_verified,
...
def validate():
    ...
    try:
        ...
        expected['recording_track_ids'] = client.get_native_recording_track_ids()
