...
def _extract_isapi_substatus(xml_text):
    """Parse an ISAPI ResponseStatus body and return (statusCode, subStatusCode).

    Hikvision firmware reuses statusString="Invalid XML Content" for multiple
    distinct rejection reasons.  The subStatusCode tells you the real cause:
      - "riskPassword"   — XML was accepted; password failed the device's
                           strength policy (too short, dictionary word, etc.)
      - "badXmlContent"  — XML was accepted; a field value was unacceptable
                           (e.g. MD5 hash sent where plaintext is expected)
    Returns ("", "") when the body cannot be parsed or the fields are absent.
    """
    try:
        root = _parse_xml(xml_text)
        if root is None:
            return "", ""
        status_code  = _text(root, "statusCode",    default="")
        sub_status   = _text(root, "subStatusCode", default="")
        return status_code, sub_status
    except Exception:
        return "", ""


def _md5_password(password):
    """Return the MD5 hex digest of a plaintext password.

    Firmware 4.83+ (PSI Alliance / std-cgi newer generation) requires the
    password field in user-create XML to be the MD5 hash of the plaintext,
    not the plaintext itself.  Older firmware (< 4.83) accepts plaintext.
    We try plaintext first and fall back to MD5 automatically.
    """
...
class DVRClient:
    def __init__(self, ip=None, port=80, username=None, password=None, use_https=False,
                 url=None,
                 admin_verification_password=None):
        if url:
            parsed = urlparse(str(url).strip())
            if parsed.scheme not in ("http", "https") or not parsed.netloc:
                raise ValueError("DVR/NVR URL must include http:// or https:// and a host")
            if parsed.path.rstrip("/") not in ("", "/ISAPI"):
                raise ValueError("DVR/NVR URL must be the device base URL, not an API path")
            self.base_url = f"{parsed.scheme}://{parsed.netloc}/ISAPI"
        else:
    ...
    def get_native_recording_track_ids(self):
        """Discover recording track IDs from the native recording API.

        Recording track IDs are not guaranteed to match the streaming channel
        collection.  Prefer the native TrackList and only probe the conventional
        xx01 IDs when the collection does not expose usable IDs.
        """
        ids = []
        try:
            response = self._get("/ContentMgmt/record/tracks")
            root = _parse_xml(response.text) if response.status_code == 200 else None
            if root is not None:
                for track in _find_all(root, "Track"):
                    node = next(
                        (child for child in list(track)
                         if _local(child.tag).lower() in ("id", "trackid")),
                        None,
                    )
                    ...
                    if value.isdigit() and int(value) > 0:
                        ids.append(value)
                if not ids:
                    for node in _find_all(root, "trackID") + _find_all(root, "trackId"):
                        value = (node.text or "").strip()
                        if value.isdigit() and int(value) > 0:
                            ids.append(value)
        except Exception as e:
            logger.info(f"Native recording track collection unavailable: {e}")

        ids = list(dict.fromkeys(ids))
        if ids:
            logger.info(f"Discovered native recording tracks: {', '.join(ids)}")
            return ids

        # Safe fallback mirrors the reference implementation: a candidate is
        # accepted only after the device returns a parseable Track document.
        for channel in range(1, 65):
            candidate = str(channel * 100 + 1)
            try:
                response = self._get(f"/ContentMgmt/record/tracks/{candidate}")
                root = _parse_xml(response.text) if response.status_code == 200 else None
                if root is not None and "track" in _local(root.tag).lower():
                    ids.append(candidate)
            except Exception:
                continue
        ids = list(dict.fromkeys(ids))
        logger.info(f"Probed native recording tracks: {', '.join(ids) or 'none'}")
        return ids
    ...
    def configure_time(self, city='', state=None, iana_timezone=None):
        """Configure NTP, timezone, and DST on the device.

        Timezone resolution order (highest priority first):
          1. iana_timezone  — explicit IANA name (e.g. "America/Chicago").
                              The ISAPI spec (§8.1.9-11) requires a POSIX TZ
                              string; this is the cleanest way to supply one.
          2. city param     — accepted as a bare IANA name for backward compat
                              (old callers that stored IANA tz in the city field).
          3. state          — US state name; looked up in STATE_TIMEZONE.
          4. Default        — America/Chicago (Central Time).
        """
        if iana_timezone and iana_timezone in TZ_TO_HIKVISION:
            iana_tz = iana_timezone
        elif '/' in (city or '') and city in TZ_TO_HIKVISION:
            iana_tz = city      # legacy: IANA name was passed in city param
        else:
            iana_tz = self.get_timezone_for_state(state or '')
...
<storage>
  <hddList>
    ...
    def configure_main_stream(self, channel_id):
        ...
        Settings: 1920×1080 progressive, H.264, CBR, 320 kbps, 8 fps
        (maxFrameRate=800).
        ...
        try:
            ...
            verified = self._verify_stream_values(
                channel_id, "1920", "1080", require_progressive=True
            )
            if not verified:
                logger.warning(
                    f"Main stream channel {channel_id}: PUT accepted but "
    ...
    def configure_sub_stream(self, channel_id):
        ...
        Primary target : 1280×720 progressive
        Fallback chain : 960×576 → 960×540 → 704×576 → 704×480 → 640×480
        ...
        Returns a dict:
          {
            ...
            'applied_resolution': '1280×720' | '<WxH>' | 'N/A',
            'note':               str   (empty on primary success),
          }
        """
        RESOLUTION_CHAIN = [
            ("1280", "720"),  # V4.83 reference target
            ("960", "576"),   # older PAL fallback
        ...
        try:
            ...
            for w, h in RESOLUTION_CHAIN:
                try:
                    ...
                    if not self._verify_stream_values(
                        sub_id, w, h, require_progressive=True
                    ):
                        raise RuntimeError(
                            f"read-back verification failed for {w}x{h}"
                        )
                    logger.info(
    ...
    def _verify_stream_values(self, channel_id, width, height,
                              require_progressive=False):
        """Read back native StreamingChannel XML and verify applied values."""
        try:
            r = self._get(f"/Streaming/channels/{channel_id}")
            root = _parse_xml(r.text)
            if root is None:
                return False
            actual_width = _text(root, "videoResolutionWidth", default="")
            actual_height = _text(root, "videoResolutionHeight", default="")
            if actual_width != str(width) or actual_height != str(height):
                return False
            if require_progressive:
                scan = _text(root, "videoScanType", default="").lower()
                if scan and scan not in ("progressive", "progressive_scan"):
                    return False
            return True
        except Exception as e:
            logger.debug(f"Stream read-back verification failed for {channel_id}: {e}")
            return False
    ...
    def configure_recording_schedule(self, channel_id):
        ...
        try:
            ...
            # Use a namespace-aware regex so we match <TrackSchedule>,
            # <std:TrackSchedule>, <hik:TrackSchedule>, etc.  A plain
            # string check ("if '<TrackSchedule>' in ...") misses the
            # prefixed variants common in 4.83+ firmware and results in a
            # silent no-op PUT that sends back unchanged XML.
            _TS_PAT = _re.compile(
                r'<(?:[^:>\s/]+:)?TrackSchedule(?:\s[^>]*)?>.*?'
                r'</(?:[^:>\s/]+:)?TrackSchedule>',
                _re.DOTALL
            )
            ts_hit = _TS_PAT.search(track_xml)
            schedule_payload = new_track_schedule
            if ts_hit:
                opening = _re.match(r"<([^:>\s/]+:)?TrackSchedule", ts_hit.group(0))
                prefix = opening.group(1) if opening and opening.group(1) else ""
                if prefix:
                    schedule_payload = _re.sub(
                        r"(<|</)(TrackSchedule|ScheduleBlockList|ScheduleBlock|"
                        r"ScheduleBlockGUID|ScheduleBlockType|ScheduleAction|id|"
                        r"ScheduleActionStartTime|ScheduleActionEndTime|DayOfWeek|"
                        r"TimeOfDay|ScheduleDSTEnable|Description|Actions|Record|"
                        r"Log|SaveImg|ActionRecordingMode)(?=[\s>])",
                        rf"\1{prefix}\2",
                        new_track_schedule,
                    )
            if ts_hit:
                updated_xml = (
                    track_xml[:ts_hit.start()]
                    + schedule_payload
                    + track_xml[ts_hit.end():]
                )
            else:
                # No TrackSchedule block yet — inject before the closing
                # </Track> tag, also namespace-aware.
                updated_xml = _re.sub(
                    r'(</(?:[^:>\s/]+:)?Track>)',
                    schedule_payload + r'\n\1',
                    track_xml, count=1
    ...
    def configure_holiday(self):
        """Create/update the holiday using the device's native XML document."""
        try:
            root = _parse_xml(self._get("/System/holidays").text)
            if root is None:
                return False
            ns = root.tag.rsplit("}", 1)[0] + "}" if "}" in root.tag else ""
            holiday = next(
                (
                    item for item in _find_all(root, "Holiday")
                    if _text(item, "holidayName", default="").lower()
                    in ("thanksgiving & black friday",
                        "thanks giving & black friday")
                ),
                None,
            )
            if holiday is None:
                holiday = ET.SubElement(root, f"{ns}Holiday")

            def set_value(parent, tag, value):
                node = _child_by_local_name(parent, tag)
                if node is None:
                    node = ET.SubElement(parent, f"{ns}{tag}")
                node.text = str(value)
                return node

            set_value(holiday, "id", "1")
            set_value(holiday, "holidayName", "Thanks Giving & Black Friday")
            set_value(holiday, "enabled", "true")
            set_value(holiday, "holidayMode", "byWeek")
            week = _child_by_local_name(holiday, "holidayWeek")
            if week is None:
                week = ET.SubElement(holiday, f"{ns}holidayWeek")
            for tag, value in (
                ("startMonth", "11"), ("startWeek", "4"),
                ("startWeekDay", "2"), ("endMonth", "12"),
                ("endWeek", "1"), ("endWeekDay", "2"),
            ):
                set_value(week, tag, value)

            self._put("/System/holidays", _serialize(root))
            verify = _parse_xml(self._get("/System/holidays").text)
            return bool(
                verify is not None
                and any(
                    _text(item, "holidayName", default="").lower()
                    in ("thanks giving & black friday",
                        "thanksgiving & black friday")
                    and _text(item, "enabled", default="").lower() == "true"
                    and _text(item, "holidayMode", default="").lower() == "byweek"
                    and _text(item, "holidayWeek", "startMonth", default="") == "11"
                    and _text(item, "holidayWeek", "startWeek", default="") == "4"
                    and _text(item, "holidayWeek", "startWeekDay", default="") == "2"
                    and _text(item, "holidayWeek", "endMonth", default="") == "12"
                    and _text(item, "holidayWeek", "endWeek", default="") == "1"
                    and _text(item, "holidayWeek", "endWeekDay", default="") == "2"
                    for item in _find_all(verify, "Holiday")
                )
            )
        except Exception as e:
    ...
    def configure_holiday_recording(self, track_id):
        """Apply 24-hour CMR to the native HolidaySchedule only.

        V4.83.x exposes holiday recording inside each recording track:
        CustomExtensionList/CustomExtension/HolidaySchedule/ScheduleBlock.
        The normal Monday-Sunday TrackSchedule must remain untouched.
        """
        path = f"/ContentMgmt/record/tracks/{track_id}"
        try:
            response = self._get(path)
            root = _parse_xml(response.text)
            if root is None:
                return {"ok": False, "detail": "Track XML could not be parsed"}
            normal = _child_by_local_name(root, "TrackSchedule")
            extension_list = _child_by_local_name(root, "CustomExtensionList")
            extension = None
            if extension_list is not None:
                extension = next(
                    (child for child in list(extension_list)
                     if _local(child.tag) == "CustomExtension"),
                    None,
                )
            holiday = _child_by_local_name(extension, "HolidaySchedule") if extension is not None else None
            block = _child_by_local_name(holiday, "ScheduleBlock") if holiday is not None else None
            if normal is None or block is None:
                return {
                    "ok": False,
                    "detail": "Native HolidaySchedule/TrackSchedule is not exposed",
                }

            ns = root.tag.rsplit("}", 1)[0] + "}" if "}" in root.tag else ""
            for old in list(block):
                if _local(old.tag) == "ScheduleAction":
                    block.remove(old)
            ...
            action = add(block, "ScheduleAction")
            add(action, "id", "1")
            start = add(action, "ScheduleActionStartTime")
            add(start, "DayOfWeek", "Monday")
            add(start, "TimeOfDay", "00:00:00")
            end = add(action, "ScheduleActionEndTime")
            add(end, "DayOfWeek", "Tuesday")
            add(end, "TimeOfDay", "00:00:00")
            add(action, "ScheduleDSTEnable", "false")
            add(action, "Description", "nothing")
            actions = add(action, "Actions")
            add(actions, "Record", "true")
            add(actions, "Log", "false")
            add(actions, "SaveImg", "false")
            add(actions, "ActionRecordingMode", "CMR")

            self._put(path, _serialize(root))

            verify_response = self._get(path)
            verify_root = _parse_xml(verify_response.text)
            verify_extension_list = (
                _child_by_local_name(verify_root, "CustomExtensionList")
                if verify_root is not None else None
            )
            verify_extension = None
            if verify_extension_list is not None:
                verify_extension = next(
                    (child for child in list(verify_extension_list)
                     if _local(child.tag) == "CustomExtension"),
                    None,
                )
            verify_holiday = _child_by_local_name(verify_extension, "HolidaySchedule") if verify_extension is not None else None
            verify_block = _child_by_local_name(verify_holiday, "ScheduleBlock") if verify_holiday is not None else None
            verify_action = _child_by_local_name(verify_block, "ScheduleAction") if verify_block is not None else None
            verified = (
                verify_action is not None
                and _text(verify_action, "ScheduleActionStartTime", "DayOfWeek", default="") == "Monday"
                and _text(verify_action, "ScheduleActionStartTime", "TimeOfDay", default="") == "00:00:00"
                and _text(verify_action, "ScheduleActionEndTime", "DayOfWeek", default="") == "Tuesday"
                and _text(verify_action, "ScheduleActionEndTime", "TimeOfDay", default="") == "00:00:00"
                and _text(verify_action, "Actions", "Record", default="").lower() == "true"
                and _text(verify_action, "Actions", "ActionRecordingMode", default="").upper() == "CMR"
            )
            return {
                "ok": verified,
                "detail": "Holiday 00:00-24:00 CMR verified" if verified else
                          "Holiday PUT accepted but read-back verification failed",
            }
        except Exception as e:
            logger.warning(f"Holiday recording track {track_id}: {e}")
    ...
    def _build_create_user_xml_from_template(self, username, password, user_level, next_id):
        ...
        next_id: integer 1-16 — the user slot to claim.  The ISAPI spec
        (§8.9.3/8.9.4) defines <id> as xs:integer "1-16"; sending 0 is
        out-of-spec and some firmware rejects it.
        """
        import re as _re
        try:
            ...
            # Pass a list of tuples — _patch_xml_fields iterates (tag, value)
            # pairs, so passing a plain dict would iterate its keys (strings)
            # and fail to unpack on any key longer than 2 characters.
            patch = [
                ('id',        str(next_id)),  # ISAPI spec: <id> must be 1-16
                ('userName',  username),
                ('userLevel', user_level),
                ('inherent',  'false'),       # not a built-in (non-deletable) account
            ]
            if self.admin_verification_password:
                patch.append(('adminPassword', self.admin_verification_password))

            user_block = _patch_xml_fields(user_block, patch)

            # Inject <password> explicitly — GET /Security/users never returns
            # password fields (write-only), so the cloned block won't contain
            # one and _patch_xml_fields cannot insert a tag that doesn't exist.
            if not _re.search(
                r'<(?:[^:>\s/]+:)?password(?:\s[^>]*)?>',
                user_block
            ):
            ...
            # Do NOT prepend an <?xml?> declaration.  This firmware's XML parser
            # counts the processing instruction as "Tag 1" and treats <User> as
            # a second root-level tag, returning:
            #   subStatusCode:    badXmlContent
            #   subStatusString:  Tag N is invalid (two root tags)
    ...
    def _detect_bond_ip_element(self, raw_userlist=None):
        """Detect which bond-IP element names this firmware uses.

        Returns (list_tag, item_tag):
          - ('bondIpAddressList', 'bondIpAddress') — ISAPI spec / older OEM firmware
          - ('bondIpList',        'bondIp')         — PSI Alliance 4.83+ / Hikvision classic

        Optionally accepts the already-fetched raw UserList XML to avoid
        an extra GET round-trip when the caller already has it.
        """
        if raw_userlist is None:
            try:
                r = self._get("/Security/users")
                raw_userlist = r.text
            except Exception:
    ...
    def _create_user_inner(self, username, password, user_level):
        ...
        # Determine the next free user slot (1-16).
        # The ISAPI spec (§8.9.3) defines <id> as xs:integer "1-16"; sending 0
        # is out-of-spec and some firmware rejects the POST with badXmlContent
        # or silently ignores the new entry.
        used_ids = set()
        for v in existing.values():
            try:
                ...
                pass
        next_id = next((i for i in range(1, 17) if i not in used_ids), None)
        if next_id is None:
            raise Exception(
                "Device has no available user slots — all 16 slots (IDs 1-16) "
                "are already occupied. Delete an existing account first."
            )
        logger.debug(f"Next available user slot: id={next_id} (used={sorted(used_ids)})")
        ...
        # Auto-detect which bond-IP element names this device uses
        # (bondIpAddressList/bondIpAddress vs bondIpList/bondIp) so the
        # fallback template mirrors the device's own schema rather than
        # guessing.  The clone strategy already handles this automatically
        # by mirroring the device's GET response, but the built-in template
        # (used when there are no existing users to clone) must match too.
        _list_tag, _ip_tag = self._detect_bond_ip_element()

        def _build_fallback_xml(pw):
            _admin_pw_tag = (
        ...
        # ── Strategy 1 (POST) then Strategy 2 (PUT-list) ──────────────────
        # Attempt with PLAIN password first (works on firmware < 4.83), then
        # automatically retry with MD5 hash if the device returns riskPassword
        # (required by PSI Alliance / std-cgi firmware 4.83 and above).
        # No <?xml?> declaration — this firmware counts it as a root-level tag.
        def _try_post(pw):
            xml = self._build_create_user_xml_from_template(username, pw, user_level, next_id)
            if xml is None:
                xml = _build_fallback_xml(pw)
        ...
        try:
            ...
            _, sub_status = _extract_isapi_substatus(post_response_body)
            if sub_status == "riskPassword":
                # Device (4.83+ firmware) requires MD5-hashed password —
                # do NOT raise yet; retry with MD5 below.
                need_md5 = True
                logger.info(
                    f"POST /Security/users for '{username}' returned riskPassword — "
                    f"device likely runs firmware 4.83+ and requires MD5-hashed passwords. "
                    f"Retrying with MD5."
                )
            else:
                logger.warning(
                    f"POST /Security/users failed for '{username}' "
                    f"(HTTP {post_http_status or 'N/A'}, subStatus={sub_status or 'N/A'}); "
                    f"falling back to PUT-list"
                )

        # ── MD5 password POST (4.83+ firmware) ───────────────────────────
        if need_md5:
            md5_pw = _md5_password(password)
            try:
                uid = _try_post(md5_pw)
                if uid:
                    logger.info(f"User '{username}' created via POST with MD5 password (4.83+ firmware).")
                    return uid
            except Exception as e_md5:
                resp = getattr(e_md5, 'response', None)
                md5_body = ""
                try:
                    md5_body = resp.text if resp else ""
                except Exception:
                    pass
                self._log_user_creation_failure(
                    username, "/Security/users [MD5]",
                    resp.status_code if resp else None, e_md5, md5_body
                )
                logger.warning(
                    f"MD5 POST also failed for '{username}': {e_md5}; "
                    f"falling back to PUT-list with MD5"
                )
                # Fall through to PUT-list with MD5
        ...
        # Also used as MD5 fallback when POST riskPassword persists.
        pw_for_put = _md5_password(password) if need_md5 else password
        try:
            uid = self._create_user_via_put_list(username, pw_for_put, user_level, next_id)
            if uid:
                if need_md5:
                    logger.info(f"User '{username}' created via PUT-list with MD5 password (4.83+ firmware).")
                return uid
        except Exception as e2:
        ...
        # ── If plain PUT-list worked but MD5 might still work ─────────────
        # (only if we didn't already try MD5 above)
        if not need_md5:
            # Also try PUT-list with MD5 in case POST rejected plain but
            # PUT requires MD5 (some hybrid-mode firmware).
            pass  # placeholder — already handled via need_md5 path above

        # All paths failed — surface the original POST error
    ...
    def _create_user_via_put_list(self, username, password, user_level, next_id):
        ...
        # 2. Build the new <User> block to inject.
        #
        #    Clone an existing <User> from the GET response and patch only the
        #    fields that must differ.  This mirrors the exact XML structure the
        #    device already produced — correct version= / xmlns= attributes,
        #    correct child element names (e.g. <bondIpAddressList> vs the older
        #    <bondIpList>), correct field ordering.  A hardcoded template risks
        #    using element names the firmware doesn't recognise, causing it to
        #    silently accept the PUT (HTTP 200) but ignore the new entry.
        cloned = _re.search(
            r'<(?:[^:>\s/]+:)?User(?:\s[^>]*)?>.*?</(?:[^:>\s/]+:)?User>',
            raw, _re.DOTALL
        )
        if cloned:
            new_user_block = cloned.group(0)
            clone_patch = [
                ('id',        str(next_id)),  # ISAPI spec: <id> must be 1-16
                ('userName',  username),
                ('userLevel', user_level),
            ...
            if self.admin_verification_password:
                clone_patch.append(('adminPassword', self.admin_verification_password))
            new_user_block = _patch_xml_fields(new_user_block, clone_patch)
            # GET never returns <password> — inject it before the closing tag
            if not _re.search(
                r'<(?:[^:>\s/]+:)?password(?:\s[^>]*)?>',
                new_user_block, _re.DOTALL
            ):
                new_user_block = _re.sub(
                    r'(</(?:[^:>\s/]+:)?User>)',
                    f'<password>{password}</password>\n\\1',
                    new_user_block, count=1
                )
            logger.debug(f"PUT-list fallback — cloned new <User> block:\n{new_user_block}")
        else:
            # No existing users to clone from — use minimal spec template.
            # Auto-detect which bond-IP element names this firmware uses from
            # the raw GET response we already have (avoids an extra round-trip).
            _list_tag, _ip_tag = self._detect_bond_ip_element(raw)
            _admin_pw_line = (
                f'    <adminPassword>{self.admin_verification_password}</adminPassword>\n'
                if self.admin_verification_password else ''
            )
            ...
            logger.debug(
                f"PUT-list fallback — no existing users to clone; using spec template "
                f"with detected element names: <{_list_tag}> / <{_ip_tag}>"
            )
    ...
    def _update_user_password(self, uid, username, password, user_level):
        ...
        """
        # No <?xml?> declaration — same reason as POST: this firmware counts
        # the processing instruction as a root-level tag, causing badXmlContent.
        xml = f"""<User>
  <id>{uid}</id>
  ...
  <userLevel>{user_level}</userLevel>
    ...
    def set_user_permissions(self, uid, username, permission_set):
        ...
        # V4.83.x expects the two native resources independently.  Sending a
        # synthetic <UserPermission> wrapper may return HTTP 200 but leaves
        # the actual local/remote permissions unchanged.
        def update_resource(section, desired):
            paths = [
                f"/Security/UserPermission/{uid}/{section}",
                f"/Security/users/{uid}/userPermission/{section}",
            ]
            last_error = None
            for path in paths:
                try:
                    ...
                    # CMS and DLT profiles also apply permissions to every
                    # returned video channel entry, exactly like the tested
                    # PowerShell implementation.
                    channel_list = _find_first(root, "videoChannelPermissionList")
                    if channel_list is not None:
                        for channel in _children_by_local_name(
                            channel_list, "videoChannelPermission"
                        ):
                            for tag in ("preview", "playBack", "palyBack", "record", "backup"):
                                if tag in desired or tag in ("preview", "playBack", "palyBack", "record", "backup"):
                                    value = desired.get(tag, desired.get("playBack", False))
                                    node = _find_first(channel, tag)
                                    if node is None:
                                        node = ET.SubElement(channel, f"{ns}{tag}")
                                    node.text = "true" if value else "false"

                    self._put(path, _serialize(root))
                    verify = self._get(path)
                    verify_root = _parse_xml(verify.text)
                    if verify_root is not None and all(
                        _text(verify_root, tag, default="").lower()
                        == ("true" if value else "false")
                        for tag, value in desired.items()
                    ):
                        return True
                    last_error = RuntimeError(f"{path} read-back did not match")
                except Exception as exc:
                    last_error = exc
                    logger.debug(f"Permission update {path} failed: {exc}")
            if last_error:
                logger.warning(f"Permission {section} update failed for {username}: {last_error}")
            return False

        local_ok = update_resource("localPermission", spec["local"])
        remote_ok = update_resource("remotePermission", spec["remote"])
    ...
    def validate_configuration(self, expected):
        ...
        _recording_track_ids = list(expected.get("recording_track_ids") or [])
        ...
        _rec_ids = _recording_track_ids or _online_channel_ids
        for _cid in _rec_ids:
            try:
                _rr = self._get(f"/ContentMgmt/record/tracks/{_cid}")
                _track_root = _parse_xml(_rr.text) if _rr.status_code == 200 else None
                if _track_root is not None and (
                    _find_first(_track_root, "TrackSchedule") is not None
                    or _find_first(_track_root, "ScheduleBlock") is not None
                ):
        ...
        try:
            ...
            if _hr.status_code == 200 and _hr.text:
                _holiday_root = _parse_xml(_hr.text)
                _holiday_items = _find_all(_holiday_root, "Holiday") if _holiday_root is not None else []
                if any(
                    _text(item, "holidayName", default="").lower()
                    in ("thanks giving & black friday", "thanksgiving & black friday")
                    and _text(item, "enabled", default="").lower() == "true"
                    for item in _holiday_items
                ):
                    _hol_verified = True
        ...
        # Required sub stream  : same encoding, 1280×720 progressive primary,
        # with documented firmware fallbacks.
        ...
        def _stream_fields_ok(stream_id, expect_w, expect_h):
            ...
            scan   = _stream_field(_sroot, "Video", "videoScanType")
            ...
            return (
                ...
                and scan.lower() in ("progressive", "progressive_scan")
            )
        ...
        for _cid in _online_channel_ids:
            ...
            try:
                _ok = any(
                    _stream_fields_ok(_sub_id, width, height)
                    for width, height in (("1280", "720"), ("960", "576"),
                                           ("960", "540"), ("640", "480"))
                )
                if _ok:
        ...
        results.append(_build_channel_check_result(
            ...
            skipped_ids=_skipped_channel_ids,
            target_desc="H.264 CBR 320kbps 8fps (1280×720 primary, with documented fallback resolutions)",
        ))
